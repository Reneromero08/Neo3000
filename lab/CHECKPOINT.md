...
**Next exact action:** `EXECUTE_NEO_EXP_0062_SINGLE_REQUEST_CATALYTIC_LATENCY`. Commit the exact static package, verify the clean head and pinned candidate/model identities, stable PID/port custody, free candidate port, and distinct absent artifact paths, then invoke the preregistered controller once at checkpoint zero under standing frontier authorization.
