...
This ledger does not claim that endpoint. Its strongest current frontier evidence is bounded by `neo-exp-0050` through `neo-exp-0062`:
...
| `T` | reuse duration across tokens, tasks, sessions, and restarts | exact checkpoint-free process-local lifecycle plus bounded fast N=1 temporal latency accepted through four counted pairs; no unbounded-duration or restart result |
...
**Next exact action:** `PROFILE_AND_CAUSALLY_LOCALIZE_THE_DOMINANT_REMAINING_SINGLE_REQUEST_HOT_PATH_WITH_THE_SAME_EXACT_CONTROL`. Use the executed timing decomposition and zero-contact source/controller audit to decide whether changing only the root boundary from `543` to the exact full `690` prompt is the narrowest `neo-exp-0063`; preserve the control, T=4 ordering, complete accounting, and no-fanout scope.
