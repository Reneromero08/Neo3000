...
## Checkpoint 2: Preserve the recovered binding-2 parent-dependence captures
...
Separate artifact `lab/ck0_balanced_opaque_rank_head_v2_binding_2_parent_dependence_forensic_recovery_1.json`, SHA-256 `110DAE908E4C1F01747C2CBBEA9F3A34BB2ACA61397D55B0648D82453A3DD975`, binds the original execution, exact failure reproduction, corrected postflight, both authenticated captures, zero-generation replay, and no-smuggle boundary. It supports only `BINDING_2_PARENT_A_INFORMATION_DEPENDENCE_SUPPORTED`, `BINDING_2_PARENT_B_INFORMATION_DEPENDENCE_NOT_SHOWN`, and `DIRECTIONAL_PARENT_A_INFORMATION_DEPENDENCE_REPLICATED_ACROSS_TWO_PRIVATE_BINDINGS`.

Parent-B dependence and bilateral parent dependence are not supported. General two-parent necessity, transfer, general catalytic inference, task advantage, superiority, SOTA, broader or restart-persistent HoloState, Deep, and automatic promotion remain locked. The recovery created or consumed no authority, made no model or HTTP request, launched no sidecar, and performed no scientific retry.
