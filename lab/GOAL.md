...
Successor attempt `relational-operation-probe-v1-attempt-2` executed exactly once from protected main `9fc7437a2624586da1bf24471bfda551d0a35a34`. It consumed one fresh authority, completed `G0-AB`, `G0-BA`, `G1-AB`, and `G1-BA` with one generation each and no retry, passed cleanup/postflight, and created no automatic follow-on. Attempt 1 remains byte-exact, terminal `INCONCLUSIVE`, and zero-generation.

Exact zero-contact replay verifies receipt/HMAC, all four captures, manifest, result, closure, the 20-event live journal, and the nine-file content-addressed archive `CC3AD55805F0F39C01CC7404B1990B4F778C215E35FFAA9C67588AB83566637E`. All four deterministic rank-zero selections match the frozen unique-intersection prediction. G0 and G1 each select the same semantic candidate under AB and BA; lexical-first, first-listed, Parent-A priority, Parent-B priority, parent order, and context-stabilization-only do not match the four outcomes.

The supported controller classification is `COMMUTATIVE_UNIQUE_INTERSECTION_LIKE_TRANSFORM_SUPPORTED_ON_TWO_MATCHED_GEOMETRIES`. The bounded scientific interpretation is `JOINT_PARENT_TRANSFORM_SELECTS_THE_UNIQUE_RELATIONAL_INVARIANT_INDEPENDENT_OF_PARENT_PRESENTATION_ORDER_ACROSS_TWO_MATCHED_PRIVATE_GEOMETRIES`. This complements `neo-exp-0042`: that panel localized when joint-parent information matters, while this probe identifies the observed joint transform as unique-intersection-like within one private binding, one fixed seed, and two identity-disjoint matched geometries.

Canonical adjudication artifact `lab/ck0_balanced_opaque_rank_head_v2_relational_operation_probe_adjudication_1.json` has SHA-256 `0EF8C0B2DD65915962ADBFE1CEE06CE6053A3FB7F55E7B3BD4C5955739F5BEED`. Canonical result `neo-exp-0043` is at line 56 with SHA-256 `C47DE48658E0C044AB676F584FF6030C0CB0A2AD5C84553B70B43EBC863C9423`. Bounded qualification passes focused adjudication `8/8`, exact publication reconstruction, changed-file compilation, canonical JSON/JSONL, private/no-smuggle scans, evaluator `28/28`, regenerated evaluator lock, and protected preflight; no complete CPU suite was run.
