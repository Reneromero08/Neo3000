...
## Checkpoint 2: Preserve terminal balanced opaque CK0 bilateral dependence evidence

`balanced-opaque-relational-carrier-v1` remains statically frozen. Full-information run `ck0-balanced-v1-full-r1` remains terminally visible and exact, and Branch-A control `ck0-balanced-v1-delete-a-r1` remains terminally classified `PARENT_A_INFORMATION_DEPENDENCE_SUPPORTED`. Reserved Branch-B control `ck0-balanced-v1-delete-b-r1` executed exactly once from protected main `3133af3b60aabe05608f51874071955467f3b6d0` with the exact frozen model, binary, carrier, schemas, seeds, one slot, one sidecar epoch, and six-request lifecycle. No retry, repair, predecessor rerun, disclosure, Deep request, or promotion occurred.

Admission verified exact protected refs, clean stable/candidate custody, exact model/binary/carrier/schema identities, evaluator lock, historical CIB0 a1-a6 and CK0 a1-a6 rehashes, exact full-information and delete-A evidence, ignored private commitments, zero tracked-secret or mapping matches, healthy sole stable PID `20388`, free port 9494, and an unconsumed delete-B reservation. Sidecar PID `33380` became ready in 26.781 seconds. All six requests completed and passed cache admission; both complete parent artifacts were constructed and privately verified with support cardinality five.

The transform received the complete unchanged parent-0 opaque relation and only the exact parent-1 commitment receipt. Operator `refine` did not rank the private singleton first; extraction did not select it and privately scored 3 / 5. Dedicated classification is `PARENT_B_INFORMATION_DEPENDENCE_SUPPORTED`. Restoration, cleanup, zero-lease accounting, stable/candidate custody, historical/full/delete-A custody, sidecar retirement, and port release passed. Manifest, result, and closure hashes are `EF2160E9CA719A4120FF7F7F226A1AD76F0BF08FE6A5C58CFFADCBCE2AB8B782`, `F0E24AAA61A2E8D0EC567F93444E08FD81EB8AFE36E2C01D7397A4D06FD46CB6`, and `880E238B877602AE3A869E2071DBDF1B0D3EE0FAF3D0D019C2D133D05C57BF90`.

Prepare only non-production `BRANCH_B_INFORMATION_DEPENDENCE_SUPPORTED_ON_BALANCED_OPAQUE_CARRIER`: under the frozen balanced opaque carrier, withholding Branch-B informative content prevented reproduction of the visible full-information singleton resolution. Because the full-information run is visible and both directional controls support dependence, also prepare `BILATERAL_PARENT_INFORMATION_DEPENDENCE_SUPPORTED_ON_BALANCED_OPAQUE_CARRIER`. This bilateral status is bounded to one carrier, one task geometry, one private alias binding, one model, one binary, and the preregistered six-request process. It is not general two-parent necessity. Transfer, general catalytic inference, advantage, superiority, SOTA, broader/restart persistence, Deep, and promotion remain locked.

The exact next action is a separately authorized static publication of the validated bounded delete-B and bilateral-dependence evidence. All three balanced-opaque reservations are consumed; no further live action is authorized.
