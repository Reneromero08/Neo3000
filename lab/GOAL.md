...
## Checkpoint 2: Prepare the bounded-controller-repair-resumable binding-2 parent-dependence causal replay
...
The future live budget is exactly two generations, one transform response per arm. One fresh external authority must bind the exact published bounded-controller-repair commit, experiment identity, binding-2 source custody and run-key commitment, `neo-exp-0041`, cross-binding adjudication, both ordered arms, model, binary, carrier, frozen scientific execution binding, initial repairable-controller binding, combined implementation, preregistration, journal, capture, and archive schemas. The authority remains bound to its original execution commit while allowing at most three linear descendant-only controller repair commits; it cannot reset authority or per-arm consumption and grants no automatic follow-on. Per-arm consumption occurs at the append-only `request-started` event. Every raw SSE line is durably recorded before transport parsing and bound into an experiment-key-authenticated capture; captured bytes replay without model contact, while a started request lacking exact capture is terminally inconclusive and is never retried. Explicit malformed, schema-invalid, transform-invalid, or authenticated-capture-invalid data may become scientific `INCONCLUSIVE`. Unexpected controller exceptions instead preserve authenticated captures and the journal through `finalization-observed`, append no adjudication, write no terminal evidence, and raise so the same command can replay deterministic parsing after repair with zero model calls.

The frozen scientific module owns the exact two arm payloads, payload hashes, immutable generation ceilings, live dispatch dependencies, source-hash gate, raw-response spool, and authenticated capture path. It rechecks the complete payload hash immediately before any contact. The repairable controller owns admission, custody, parsing, verification, adjudication, and publication. Before any contact after a controller change, it proves descendant-only linear history, limits the cumulative repair count to three, admits only declared controller paths, verifies the frozen scientific binding and exact request hashes, preserves the original receipt and existing captures byte-for-byte, and appends one authenticated `controller-repair-observed` transition. Re-observing the same commit is idempotent; a non-descendant, fourth repair, scientific mutation, or unauthorized path fails before contact.

After a process restart, a missing finalization or request-custody event invokes custody-only reconciliation: no sidecar is launched, no model request is issued, the prior sidecar must be absent, port 9494 must be free, stable/candidate/source/publication/historical/evidence custody must remain exact, and no unauthorized runtime path may exist. A passing reconciliation appends `request-custody-observed` in `restart-reconciled` mode where needed and records `finalization-observed` in `restart-custody-reconciliation` mode. Failed reconciliation is fail-closed and never regenerates a captured arm. Both rank-zero selections must still freeze successfully before either private mapping or score is computed.

Frozen scientific execution binding is `F084C1BE1FB5E90C33DEB15F60D969F8B24B23DB13E27A6066360B9CC0FFA1F3`; initial repairable-controller binding is `E2A6FD65816E19D3ECAE90DFF9C88547E6D13C66C62339D18DBA3A2AB8B578A9`; combined implementation binding is `8B0BC8EF5ACE3C28A91995B2E30FD391DDD210B02FE68B1DAE5F02FA57DCBFA0`. Preregistration artifact and canonical document hashes are `A32E445EC136E5BC863E09EE177EE5902A20D8176E810B3B238F75A615698511` and `C403930014945B3AEE647E6E6A755286B0F6297C17EF8D1AFD81375CB90EB6F9`. Authority object and receipt schema hashes are `4ED7A597E8AC1FC4BDB2466CA2229B479C18ED37699CF0505142C2AACD55E2A5` and `1B6BB1D13CCA5430E9DAF26ADED9EACE2F50A64E2C8A4DAA5BD21738306D3B9F`; capture and response schema hashes are `7FB8EBCD5A27654031E94EE5C7A5BFA9090EA5015B151535F39B1CF4E48317C7` and `B36864F84116E64D96775452265513E7AB46521418EB376CC17872492B281E27`. Arm request hashes remain `260F18D1C143C5D61ED775BB85F297D0A96E1507451347BED517BDAEDEE701A8` and `2B1124B9D63BC6AF4BF22D25D30606034F26466EEAFB8CF9081ECCC7C242395E`.
...
**Next exact action:** `After the bounded-controller-repair commit publishes, separately authorize ck0-balanced-v2-rank-head-b2-parent-dependence-r1 with one fresh external authority ID bound to the exact published commit and canonical preregistration.`
