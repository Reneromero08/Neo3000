...
`neo-exp-0083` is the exact successor. It preserves every scientific mechanism, request, order, cost, identity, gate, runtime, and claim lock from 0082 and changes only pair postprocessing: extract each numeric `pair` field before route lookup and require exactly primary/control. A pair-1/pair-10 regression test closes the prior failure class. Static audit 7/7, dedicated 9/9 and focused-plus-compatibility 44/44 tests, Python compilation, diff check, and independent differential review pass without endpoint, model, or GPU contact. Bind `frontier-attempt-0114`; 0083 remains unconsumed.
...
**Current exact action:** follow the single active cursor at the top of `TASKS.md`: `EXECUTE_NEO_EXP_0083_CUDA_ROOT_PLUS_HOST_TERMINAL_LOGITS_CONTINUATION_T16_ONCE`. Every later next-action statement in this file is a historical boundary record and has no current authority.
