...
`balanced-opaque-transform-only-contrastive-relational-operation-probe-v1` is implemented and statically preregistered from protected starting main `b6600c2800a80c0892bef15bca5ee2ce577ef34f`. It asks whether the joint-parent transform behaves like commutative unique-intersection recovery, parent-order or parent-role composition, lexical/list-position selection, generic context stabilization, or no identifiable operation.

The minimum design contains exactly four future transform-only requests under one existing ignored private root and one newly domain-separated private binding: `G0-AB`, `G0-BA`, `G1-AB`, `G1-BA`. G0 and G1 are identity-disjoint five-by-five geometries with different unique shared candidates. Each shared candidate occupies non-first positions 4 and 3 in Parent A and Parent B. AB/BA swap only parent presentation order. All four assignments are exactly 617 canonical bytes and 254 pinned-tokenizer tokens. The fixed seed is `830800182`; deterministic extraction freezes rank zero before private comparison.

Five predictions are frozen before outcomes for every request: unique intersection, lexical first, first listed, Parent-A priority, and Parent-B priority. Public tracked state contains only private HMAC commitments, equality structure, exact request hashes, length facts, and claim limits. It contains no candidate identity, mapping, root, ranking, raw authority, or local path.

Canonical preregistration artifact `lab/ck0_balanced_opaque_rank_head_v2_relational_operation_probe_1.json` has SHA-256 `01C3C084CC0D88BB69E2DF09321E4B4F69CC1B193887EBB24426D8292A6095B3`. Frozen scientific binding is `D9300869CE6C903D309899F5E9AFF02D4C7F9B0D815EC877CCC3BEA5DB67DE7C`; controller binding is `18425127C5B7527DC69158E49268E7B0AB10AAEBA313E6ABA9AEA4E50C306143`.

Bounded qualification passes focused probe `12/12`, complete relevant rank-head-v2 family `212/212`, exact preregistration reconstruction, changed-file compilation, canonical JSON/JSONL, private/no-smuggle scanning, evaluator `28/28`, regenerated evaluator lock, and protected preflight. The complete CPU suite was intentionally not run.

No authority was created or consumed. No sidecar was launched. No model request, generation, scientific result, publication record, or follow-on architecture was created. The completed position-seed evidence remains unchanged. Formal algebra, position-independent bilateral dependence, general two-parent necessity, cross-binding generalization, transfer, general catalytic inference, task advantage, superiority, SOTA, broader or restart-persistent HoloState, Deep, and automatic promotion remain locked.

**Next exact action:** separately authorize exactly one live execution of the four frozen requests from the final published static commit with a fresh external authority ID. Maximum one generation per request and four total; no retry or automatic follow-on.
