...
-> Track B mechanism discovery: CK0 a2 preserved collapsed / unresolved-support carrier statically implemented / one separately authorized live run pending
```
...
- **Current boundary:** Track A claim verification is paused; CIB0 a1-a6 and CK0 a1/a2 remain frozen. A separately authorized CK0 carrier revision preserves the exact six-request kernel while replacing impossible different unique branch winners with bounded complementary unresolved support sets. Public-only deterministic selection froze Tier-1 profile `complementary-unresolved-public-v1`; one implementation push and one no-retry live cycle are pending. Regardless of classification, task advantage, superiority, SOTA, broader HoloState, restart persistence, Deep, and automatic promotion remain locked.
