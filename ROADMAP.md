...
`balanced-opaque-five-support-two-worker-synthesis-probe-v1` is now statically preregistered as the minimum worker-relation capability test. It freezes exactly three future generations: isolated `worker-A`, isolated `worker-B`, then `synthesis-AB`. Each worker must model-generate its complete five-member relation in the shared opaque namespace; the controller may validate and outcome-independently normalize an exact set but may not replace, add, or remove members. Synthesis receives only the two authenticated relation artifacts and the generic rank-head-v2 transform carrier. The hidden-exact candidate is consulted only after public profile freeze, and live scoring is deferred until all three generations complete.

The hidden utility gate passes `16/16`, worker payloads are matched at 16,393 canonical bytes and 5,566 pinned-tokenizer IDs, and the static preregistration is `9FD644C53DAAB27CF4607BB43D1FDEA6ECD88F8E2CBBC34FADDEF10194ADA29B`. No authority, sidecar, model request, generation, capture, result, archive, or publication record exists. The next bounded action is `SEPARATELY_AUTHORIZE_ONE_LIVE_BALANCED_OPAQUE_FIVE_SUPPORT_TWO_WORKER_SYNTHESIS_PROBE` against the final published static commit. Another task, seed, binding, baseline, control, generalized swarm layer, or follow-on design remains off-path.
