...
-> Track B mechanism discovery: CK0 a3 completed / CATALYTIC_KERNEL_VISIBLE / bounded uncertainty reduction only / no claim change
```
...
- **Current boundary:** Track A claim verification is paused; CIB0 a1-a6 and CK0 a1-a3 are frozen. The separately authorized Tier-1 `complementary-unresolved-public-v1` run completed the exact six-request kernel and classified `CATALYTIC_KERNEL_VISIBLE`: model branch rankings collapsed, but different controller-normalized public support sets were bound into a transform that selected their unique singleton resolution and restored cleanly. This establishes bounded relational uncertainty reduction in one run only. Task advantage, superiority, SOTA, general catalytic inference, broader HoloState, restart persistence, Deep, and automatic promotion remain locked.
