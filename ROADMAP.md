...
Canonical decision artifact `lab/holostate_v1_warm_trajectory_successor_selection_1.json`, SHA-256 `C14F334F95D5100FBBA0CD690388D1DD1BAA17DC5351C9E6773F82E6D05E6B92`, binds the selection. The next exact action is `PREPARE_INHERITED_TASK_A_CARRIER_EVALUATION`: freeze the same four-pair matched panel with `Task A -> inherited catalytic Task B -> immediate closure/readdress -> fresh direct Task B`, no separate materialization, one slot, one sidecar epoch, at most 12 generations, and exact cache/fresh accounting. General catalytic inference, complete catalytic lifecycle, exact restoration, transfer, restart persistence, superiority, SOTA, and automatic promotion remain locked.
