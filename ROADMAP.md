...
-> Track B mechanism discovery: ACTIVE through repeatable Catalytic Inference Bench 0
```
...
- **Current boundary:** Track A claim verification is paused. Reactivation requires a smaller bench to repeatedly show root borrowing, relational transformation, extraction from transformed lineage, restoration, and behavior distinguishable from direct/independent/serial baselines. Track B mechanism discovery is active and explicitly non-claiming: Catalytic Inference Bench 0 uses one physical slot and a 13-request `cs1-task-06` process, pins the task/root identities, binds dependent outputs to sent-context and consumed-artifact hashes, binds evidence-linked parent-rank deltas to explicit relation-graph edges consumed by extraction, requires trusted cleanup/lease/custody restoration and terminal state closure, persists bounded metadata only, scores hidden data only after extraction, and cannot unlock production claims.
