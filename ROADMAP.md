...
Attempt 4 executed once from `748b3fff8ef59fc78069fe21a463a8eb282a48af` and terminated `INCONCLUSIVE` after one authenticated Task-A generation when its first explicit zero-token carrier materialization produced a completion token. No Task B or carrier operation completed, retry count remained zero, and cleanup/postflight passed. The terminal receipt, manifest, journal, result, closure, capture, and archive remain preserved at `9631B4F62528B6D099255E5B22F34A77F2A9F733DE7F74877CD2BA57B65B4E4B`, `EE9DAB0A9E8062292E7B519CC75410B81089524FAB2E554D839E680B9FEC185A`, `E802460EC42A64E55ECC1A9147C160C5F69851D70C9C02B506CDD61A19A5950D`, `05840E52327BE306D5781C6FFE81A040A50E89301C3860C6945F58CC97A59207`, `3E3FF03DA99FAED8BA19650B7E6D42A9B30E77902FE57F9186E8BB75679093A0`, `6CFA87719F336EF8B4841E0ED4A866B65EDCEFD5C7F2C5B4AAE70492B8B955A9`, and `BEF9D3E36080D29CDE8F33A7C5CE842F1D774E46093E2F740784A7C4F18768DD`.

Attempt 5 changes only the demonstrated runtime defect: after prompt evaluation, `n_predict=0` now completes before sampling, while zero-count timing denominators remain defined. The CUDA candidate binary identity is `EFA521C4DC4189C89CC71B34CDB46079143857A4D49148E2D4411D3F7599FEE5` / `160 (89762c0)`; a real-model live regression reports `9` prompt tokens, `0` generated tokens, empty output, and stop type `limit`. A zero-contact preflight then exposed that the generic adapter admitted only the historical stable binary, so Attempt 5 now explicitly preregisters the candidate hash/version and rehashes that exact identity under the existing replacement-denying runtime locks. Preregistration artifact/file are `9BDB84694638A0066E0572E3FE2C28B0CF8240551687219FC1650E1878B4F93D` / `AB5BDB8DD8961CAFF131CCFA640B08356B370DEF9DA3B7FA78E2DDDAF136A1B6`; controller is `38DDFB4EBBCB281E6B3F68CEB8994112ABDB05F8134909E185258FFD97FAF915`; frozen scientific, scorer, and resource bindings are unchanged.

Attempt 5 executed once from `a38e86e45e0cfc20e2882167755a8d7df91ecd51` and preserved terminal `INCONCLUSIVE` after one authenticated Task-A capture and zero admitted carrier operations. The repaired server returned explicit `predicted_n=0`; the controller falsely treated prompt-progress `tokens:[0]` markers as generated tokens and used their count when the explicit counter was zero. Cleanup passed, while postflight still invoked the historical stable-only binary verifier. Receipt/result/closure/archive hashes are `DB7C0C334429500F4E1C8B813D0BF68704BD9E8692CD7201B0917971E451BC79`, `13CE78DD00182BD74D05AFE94134A6BEEB6B8BBA8618419C588637F184802825`, `A2F314E7AF5B4255B75673A1AF9F611AD8A0764CD0CB93D4845E8D460F3A6162`, and `AA40B8BBA2F4872C84FA7104264BBD9162F4196777FF071DFF7F0006D972D494`.

Attempt 6 changes only those demonstrated controller defects: explicit zero-valued terminal counters are authoritative, prompt-progress markers cannot enter generated-token evidence, and postflight reverifies the identity admitted by preflight. The raw stream parser is now bound into controller and resource-accounting identities. A local real-model streamed regression returned `11` prompt tokens, zero completion tokens, no generated token IDs, and stop type `limit`. Preregistration artifact/file are `35EE6DE54B1146247C305AB447C2B7EDB173781AAD060BF17FCD6845E16BFDA4` / `57E12AD45DC7D4CD99DEB13AC7EA636B77C16E72BF0334B447D6A4D26FCC7619`; controller/resource identities are `B766AE4371FB9DCCA6664EB0D5335C60D5483206BE2FC95D7C330F9B4720758E` / `1FB748E2014692AE600AE94EC68FF0F15E9BFFC946F6CB835F6777882B3A77BA`; frozen scientific and scorer identities remain unchanged.

Attempt 6 executed once from `3eaa86ffb690611f3549272bdd367c778ddf8da2` and completed the first full task pair. Catalytic Task B reused all `1004` checkpoint tokens while direct Task B reused zero of `1180`; immediate closure readdress reused `872/1004`. The exact closure gate therefore remained false. Cleanup and repaired postflight passed, but the controller incorrectly treated that scientific gate failure as an execution exception and stopped before the remaining nine model generations, contrary to the frozen no-semantic-early-stop law. Receipt/result/closure/archive hashes are `E89D45A5093F6BB0AE164BC585550172E210F84CE2CA36BE759DEF403091CD7B`, `33DCC2DBB750D956C12E879FF03DE5F56010E7652AA1C493E2B35E88CBA9CB4D`, `EE1BA952241BA824457DDDFA192DAA47078FF8C2482CE1ABA26CC8DC0800EC42`, and `6AC053C53C48C9552283D6D3F75CFC635CDCF40540AA5369A2342ABE21C49AD5`.

Attempt 7 changes only that stop behavior. Each pair's actual reuse and closure booleans are authenticated and retained, execution always proceeds to the next frozen pair after a scientifically valid false gate, and the unchanged terminal decision law still returns `INCONCLUSIVE` if any exact closure fails. Preregistration artifact/file are `9370CD0A2A041BD46A765728B1A93B622BB66BD3EEFF71D45E80189C453C1002` / `469062E6AEEE953988AF4DC035E4A2155673063B2E3C6D9699294CE7799BE68A`; controller is `AC85A6FDADE3D4515B5C3DF7A9766BF74CC8D0EDF57192D028FF041C5939A8E1`; frozen scientific, scorer, resource, and closure identities remain unchanged.

Attempt 7 executed once from protected commit `20798cb5712decea8a0acb158ceeaa0a012c7844` and completed all 12 one-generation requests plus eight zero-output carrier operations without retry. The live controller correctly remained terminal `INCONCLUSIVE` because exact immediate closure failed. Zero-contact adjudication nevertheless supports the bounded component result `PROCESS_LOCAL_WARM_TRAJECTORY_EXACT_CHECKPOINT_REUSE_REPLICATED`: all four catalytic continuations reused their exact process-local checkpoint, direct reuse stayed zero, and Task-B accuracy was `4/4` on both routes.

Closure is the measured remaining barrier. Across four pairs, closure readdress reused `3657/4185` checkpoint tokens and required `528` fresh tokens. Carrier materialization plus catalytic suffix exactly equaled direct replay (`4185 + 723 = 4908`), while closure raised the complete catalytic cycle to `5436`; therefore complete-cycle fresh-token advantage is not supported. Canonical adjudication artifact `lab/holostate_v1_warm_trajectory_related_task_evaluation_v1_attempt_7_adjudication_1.json`, SHA-256 `2E7399FA57EE31E6B83A8B76F8065D6F5E1682BA91EB561C9EDB2439DB58A925`, and `neo-exp-0048` at line 61, SHA-256 `DE406303E7DBAB2D8E8E95B94A5ED17DD52F1FFF5E3141112077833C58ED0A6E`, preserve this bounded conclusion without rewriting the live result.

Static successor selection chooses `INHERITED_TASK_A_CARRIER_SELECTED`. The existing one-slot runtime already leaves Task A resident and can issue an immediate cache-enabled Task-B continuation, so the minimum causal change is to remove the four redundant fresh materializations. On Attempt-7 accounting this projects inherited Task-B suffix plus measured closure at `723 + 528 = 1251` fresh tokens versus direct replay `4908`, a projected difference of `3657`; this is a preregistration target, not an executed result. Exact inherited-prefix reuse must be reconstructed before contact and observed at runtime. Pure closure/root-copy is not selected because perfect closure alone only restores equality and the current launch exposes no frozen root-copy path; two-branch amortization remains a secondary fallback because it adds branches before testing the simpler inherited carrier.

Canonical decision artifact `lab/holostate_v1_warm_trajectory_successor_selection_1.json`, SHA-256 `C14F334F95D5100FBBA0CD690388D1DD1BAA17DC5351C9E6773F82E6D05E6B92`, binds the selection.

The selected evaluation `holostate-v1-inherited-task-a-carrier-evaluation-v1` executed once from protected commit `26aa75a27ee17d625c0965a1ccb96373d925f735`. Its immutable live classification remains `PROCESS_LOCAL_INHERITED_TASK_A_CARRIER_EXACT_REUSE_NOT_SUPPORTED`: all 12 model generations and four zero-output closure operations completed without retry, but none of the four inherited continuations admitted the complete statically derived lexical prefix.

Zero-contact replay supports the bounded component result `PROCESS_LOCAL_INHERITED_TASK_A_PARTIAL_PREFIX_REUSE_REPLICATED`. Expected/admitted inherited prefix counts are `1004/769`, `1052/789`, `1031/744`, and `1098/864`; inherited and direct Task-B accuracy are both `4/4`. Including actual closure, the inherited marginal route used `2761` fresh prompt-plus-completion tokens versus direct Task B `4908`, exact cross-products `11044 < 19632`, a measured reduction of `2147/4908 = 43.74490627546862%`. Exact restoration remains unsupported because closure required `1019` fresh tokens.

Exact offline Qwen3.5 tokenization and source-bound server analysis confirm `EXECUTABLE_REUSE_LIMITED_BY_MIXED_CHECKPOINT_AND_TOKENIZATION_BOUNDARIES_CONFIRMED`. The retained Task-A prompt and newly rendered Task-B prompt first diverge four tokens before Task-A prompt end at the disabled-thinking scaffold (`<think>`) versus captured assistant JSON. The admitted DSV4 runtime uses `n_ubatch=128`, cannot reconstruct arbitrary partial sequence removal, restores a prior safe checkpoint, and reports post-restore `n_past`; therefore every pair loses another 128 tokens and the exact prompt-to-admitted shortfall is `4 + 128 = 132`. The five-token completion identity is one terminal EOG token plus four contextual chat-template retokenization tokens, not five missing semantic tokens.

Canonical adjudication artifact `lab/holostate_v1_inherited_task_a_carrier_evaluation_v1_adjudication_1.json`, SHA-256 `CC7658ECA40C4A586FE6AF56B9B8751080BEAA0A53BD1B8C30121F4B9CEB3882`, and `neo-exp-0049` at line 62, SHA-256 `B133161184777A50BB90430515CFD74EE4EE9FFC394B720ECA69DDBE2ECE88A6`, bind the result without rewriting its live negative exact-reuse verdict.

Static source and evidence analysis selects `RUNTIME_NATIVE_EXECUTABLE_CARRIER_BOUNDARY_SELECTED`. Under the exact bound DSV4 runtime, the executable carrier is the source-predicted admitted `n_past`: the complete retained live prefix when the source predicate requires no rollback, otherwise the latest eligible safe checkpoint. It is derived from exact old/new token geometry, memory-removal capability, model-derived `n_swa`, retained-state position, checkpoint inventory/provenance, message spans, scheduling state, and batch configuration, not from a lexical target. The four observed cases simplify to raw common prefix minus `n_ubatch`, but that shortcut is not a universal runtime law. The complete derivation algorithm and runtime inputs must be frozen before an experiment; for model-authored Task-A state, the exact numeric boundary is committed after authenticating Task A and before any downstream branch contact, then tested against observed admitted-cache telemetry.

Chat-template alignment remains a low-risk adjunct, not a standalone successor. Removing the four-token disabled-thinking scaffold is statically feasible, but rendered text does not guarantee exactly four additional reused tokens or the same 128-token rollback. A stronger controller-only route is source-supported: authenticate emitted token IDs, omit only the terminal EOG that was returned but never decoded into retained state, and make the next request an exact token-array strict extension. With the bound Qwen3.5-MoE `n_swa=0`, that geometry can bypass DSV4 checkpoint restoration and inherit the full retained terminal prefix; it remains unexecuted and proves neither branch restoration nor closure. Engine-level terminal-checkpoint and sequence-copy repairs are deferred, while the existing full-state snapshot surface may be evaluated during future branch isolation before any new engine architecture.
...
The next exact action is `PREPARE_MULTI_BRANCH_RUNTIME_NATIVE_CARRIER_EVALUATION`. The next evidence must ask whether one authenticated executable carrier can support multiple distinct held-out branches with truthful per-branch reuse, direct controls, actual fresh-token accounting, and separately measured closure. General catalytic inference, complete catalytic lifecycle, exact restoration, transfer, restart persistence, superiority, SOTA, and automatic promotion remain locked.
