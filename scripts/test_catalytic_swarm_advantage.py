...
class ArmPlanTests(unittest.TestCase):
    ...
    def test_all_arms_share_exact_budget_role_and_seed_sequence(self) -> None:
        ...
        for plan in plans:
            ...
            self.assertEqual(plan.max_completion_tokens, MAX_COMPLETION_TOKENS_PER_ARM)
            self.assertFalse(plan.hidden_feedback_visible)
            self.assertFalse(plan.automatic_promotion)
            self.assertTrue(all(turn.max_tokens == MAX_TOKENS_PER_REQUEST for turn in plan.turns))

    def test_topologies_are_distinct_and_locked(self) -> None:
        chain = build_advantage_arm_plan("serial-chain")
        independent = build_advantage_arm_plan("independent-ensemble")
        ...
        self.assertEqual(chain.turns[1].parent_turn_ids, (chain.turns[0].turn_id,))
        self.assertTrue(all(not turn.parent_turn_ids for turn in independent.turns))
        ...
        self.assertTrue(all(not turn.verifier_feedback_visible for turn in sparse.turns))
        self.assertTrue(all(turn.verifier_feedback_visible for turn in verified.turns))
        self.assertEqual(sum(turn.phase == "proposal" for turn in sparse.turns), 16)
        self.assertEqual(sum(turn.phase == "evidence" for turn in sparse.turns), 8)
        self.assertEqual(sum(turn.phase == "critique" for turn in sparse.turns), 6)
        self.assertEqual(sum(turn.phase == "synthesis" for turn in sparse.turns), 2)

    def observation(self, turn, candidate_id: str = "C00", public_passed: int = 3) -> TurnObservation:
    ...
    def test_assignment_uses_six_fixed_slots_and_no_hidden_data(self) -> None:
        ...
        parents = tuple(
            self.observation(next(item for item in verified.turns if item.turn_id == parent))
        ...
        sparse_parents = tuple(
            self.observation(next(item for item in sparse.turns if item.turn_id == parent))
            for parent in sparse_turn.parent_turn_ids
        )
        sparse_payload = json.loads(render_turn_assignment(self.task, sparse_turn, sparse_parents))
        self.assertEqual(
    ...
    def test_wrong_parent_context_is_rejected(self) -> None:
        ...
        with self.assertRaises(AdvantageControlError):
            render_turn_assignment(self.task, chain.turns[1], ())

    def test_response_parser_is_exact(self) -> None:
        self.assertEqual(parse_candidate_content('{"candidate_id":"C00"}', self.task), "C00")
        with self.assertRaises(AdvantageControlError):
            parse_candidate_content('{"candidate_id":"C00","extra":1}', self.task)
        with self.assertRaises(AdvantageControlError):
...
class RunnerAndComparisonTests(unittest.TestCase):
    ...
    def fake_runner(self, answer_id: str):
        def run(turn, public_root: str, assignment: str) -> dict[str, Any]:
            ...
            return {
                "content": json.dumps({"candidate_id": answer_id}, separators=(",", ":")),
                "prompt_tokens": 100,
                "cached_prompt_tokens": 80,
                "fresh_prompt_tokens": 20,
                "completion_tokens": 8,
    ...
    def test_runner_completes_32_turns_under_budget(self) -> None:
        ...
        outcome = run_advantage_arm(
            build_advantage_arm_plan("verified-swarm"),
            task,
    ...
    def test_runner_fails_closed_on_reasoning_or_budget(self) -> None:
        ...
        def bad_runner(turn, public_root, assignment):
            result = self.fake_runner(task.answer_candidate_id)(turn, public_root, assignment)
    ...
    def complete_outcome(
        ...
        *,
        ...
        wrong = next(item.candidate_id for item in task.candidates if item.candidate_id != answer)
        candidate = answer if success else wrong
        if hidden_passed is None:
            hidden_passed = 12 if success else 0
        return ArmOutcome(
            ...
            final_public_passed=7 if success else 0,
            final_public_total=7,
            final_hidden_passed=hidden_passed,
            final_hidden_total=12,
    ...
    def test_budget_parity_accepts_equal_and_rejects_skew(self) -> None:
        task = self.suite.tasks[0]
        equal = [self.complete_outcome(task, arm, success=True) for arm in ARMS]
    ...
    def make_comparisons(self, verified_successes: int, baseline_successes: int):
        ...
        for index, task in enumerate(self.suite.tasks):
            outcomes = []
            for arm in ARMS:
                success = index < verified_successes if arm == "verified-swarm" else index < baseline_successes
                outcomes.append(
                    self.complete_outcome(
                        task,
                        arm,
                        success=success,
                        hidden_passed=12 if success else 2,
                    )
                )
    ...
    def test_suite_advantage_requires_locked_margin(self) -> None:
        accepted = classify_suite_advantage(self.make_comparisons(7, 4))
        self.assertEqual(accepted.verdict, "reviewable-accept")
        self.assertEqual(accepted.task_advantage, "reviewable-accept")
        self.assertFalse(accepted.automatic_promotion)

        no_advantage = classify_suite_advantage(self.make_comparisons(6, 5))
...
class StaticSafetyTests(unittest.TestCase):
    def test_control_modules_have_no_network_or_subprocess_surface(self) -> None:
        for filename in ("catalytic_advantage_tasks.py", "catalytic_swarm_advantage.py"):
