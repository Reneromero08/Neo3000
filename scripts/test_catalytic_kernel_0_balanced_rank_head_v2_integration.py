...
class RankHeadV2IntegrationTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.source_private = balanced.PrivateBinding.from_secret(bytes(range(32)), balanced.BINDING_1)
        cls.spec = integration.RUN_SPECS[integration.BINDING_1_RUN_ID]
        cls.private = integration.runtime_private_from_source(cls.source_private, cls.spec)
        cls.runtime = integration.RankHeadV2Runtime(repository=Path("."), spec=cls.spec, private=cls.private)
        cls.winner = cls.private.internal_to_alias[balanced.EXPECTED_FULL_SUPPORT[0]]
        cls.alternatives = [alias for alias in balanced.ALIASES if alias != cls.winner]
    ...
    def executor(self, winner: bool):
        ...
        def execute(stage: str, payload):
            ...
            if stage in {"borrow", "restore"}:
                return json.dumps({"accepted": True, "carrier_id": v2.V2_CARRIER_ID})
            if stage in {"branch-a", "branch-b"}:
                return json.dumps({"ranking": ["K00", "K01", "K02"]})
            ranking = [self.winner, *self.alternatives[:2]] if winner else [self.alternatives[0], self.winner, self.alternatives[1]]
    ...
    def test_six_logical_stages_five_model_requests(self) -> None:
        ...
        self.assertEqual(result["terminal_classification"], integration.VISIBLE_CLASSIFICATION)

    def test_nonwinner_rank_head_remains_collapsed(self) -> None:
        _, executor = self.executor(False)
        result = integration.execute_logical_cycle(self.runtime, executor)
        self.assertEqual(result["terminal_classification"], integration.COLLAPSED_CLASSIFICATION)
        evaluation = result["artifacts"]["extract"]["controller_private_evaluation"]
    ...
    def test_existing_alias_mapping_is_preserved_and_run_key_changes(self) -> None:
        self.assertEqual(self.private.alias_to_internal, self.source_private.alias_to_internal)
        self.assertNotEqual(self.private.run_key(self.spec.run_id), self.source_private.run_key(self.spec.source_full_run_id))
    ...
    def test_exact_six_file_binding(self) -> None:
        integration.require_exact_implementation_paths(integration.REQUIRED_IMPLEMENTATION_PATHS)
        with self.assertRaises(integration.RankHeadV2IntegrationError):
            integration.require_exact_implementation_paths(integration.REQUIRED_IMPLEMENTATION_PATHS[:-1])
    ...
    def test_v2_carrier_contains_no_extract_schema(self) -> None:
        ...
        self.assertEqual(set(root["response_schemas"]), set(integration.MODEL_REQUEST_STAGES))
