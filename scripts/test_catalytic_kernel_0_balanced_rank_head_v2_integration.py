...
class RankHeadV2IntegrationTests(unittest.TestCase):
    ...
    def test_six_logical_stages_five_model_requests(self) -> None:
        ...
        result = integration.simulate_logical_cycle(
            self.runtime,
            executor,
        ...
        self.assertEqual(
            result["simulation_classification"],
            integration.VISIBLE_CLASSIFICATION,
        )
        self.assertEqual(result["status"], "static-simulation-only")
        self.assertEqual(result["physical_lease_count"], 0)
    ...
    def test_v2_run_keys_are_domain_separated_across_source_bindings(self) -> None:
        second_spec = integration.run_spec(integration.BINDING_2_RUN_ID)
        second_source = balanced.PrivateBinding.from_secret(
            bytes(reversed(range(32))),
            balanced.BINDING_2,
        )
        second_private = integration.runtime_private_from_source(
            second_source,
            second_spec,
        )
        self.assertNotEqual(
            self.private.run_key(self.spec.run_id),
            second_private.run_key(second_spec.run_id),
        )
        self.assertNotEqual(
            second_private.run_key(second_spec.run_id),
            second_source.run_key(second_spec.source_full_run_id),
        )
    ...
    def test_direct_logical_execution_surface_fails_closed(self) -> None:
        with self.assertRaisesRegex(
            integration.RankHeadV2IntegrationError,
            "direct logical execution is forbidden",
        ):
            integration.execute_logical_cycle(self.runtime, self.executor(True)[1])
