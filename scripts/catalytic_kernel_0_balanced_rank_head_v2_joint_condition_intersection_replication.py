#!/usr/bin/env python3
"""Joint-condition replication of the CK0 unique-intersection-like transform.

The static path creates one fresh private binding, derives one fixed seed from
the design and starting commit, freezes two outcome-independent geometries and
five competing predictions, and writes one zero-execution preregistration. The
live path remains closed until a later external one-shot authority binds the
published static commit.
"""
...
class JointConditionIntersectionReplicationError(ValueError):
    """The probe geometry, private custody, or one-shot contract changed."""


DESIGN_ID = scientific.DESIGN_ID
STARTING_PROTECTED_MAIN = "b955b26bad38e8e9f9d823e3e15bf461cb325019"
PREREGISTRATION_PATH = Path(
    "lab/ck0_balanced_opaque_rank_head_v2_joint_condition_intersection_replication_1.json"
)
PRIVATE_ROOT_PATH = Path(
    "state/catalytic_kernel_0_private/joint-condition-intersection-replication-v1.secret"
)
PRIVATE_RECEIPT_PATH = Path(
    "state/catalytic_kernel_0_private/joint-condition-intersection-replication-v1.creation.json"
)
PREVIOUS_PRIVATE_ROOT_PATHS = (
    Path("state/catalytic_kernel_0_private/balanced-opaque-relational-carrier-v1.secret"),
    Path("state/catalytic_kernel_0_private/balanced-opaque-relational-carrier-v1-binding-2.secret"),
    Path("state/catalytic_kernel_0_private/.balanced-opaque-position-seed-crossover-v1.secret"),
)
STATE_ROOT = Path("state/catalytic_kernel_0/joint_condition_intersection_replication_v1")
ARCHIVE_ROOT = Path("state/catalytic_kernel_0/joint_condition_intersection_replication_evidence_archive/v1")
AUTHORITY_RECEIPT_PATH = Path(
    "state/catalytic_kernel_0_authority.joint-condition-intersection-replication-v1.authority.consumed.json"
)
...
SOURCE_FIXED_TRANSFORM_SEED = 830800182
FIXED_SEED_DERIVATION_DOMAIN = (
    b"ck0-rank-head-v2/joint-condition-intersection-replication/fixed-seed-v1\0"
)


def derive_fixed_transform_seed() -> int:
    canonical_seed_material = (
        DESIGN_ID.encode("utf-8") + b"\0" + STARTING_PROTECTED_MAIN.encode("ascii")
    )
    digest = hashlib.sha256(FIXED_SEED_DERIVATION_DOMAIN + canonical_seed_material).digest()
    return 1 + int.from_bytes(digest[:8], "big") % ((1 << 31) - 2)


FIXED_TRANSFORM_SEED = derive_fixed_transform_seed()
if FIXED_TRANSFORM_SEED == SOURCE_FIXED_TRANSFORM_SEED:
    raise RuntimeError("derived replication seed collides with the source seed")
...
SOURCE_ADJUDICATION_SHA256 = (
    "0EF8C0B2DD65915962ADBFE1CEE06CE6053A3FB7F55E7B3BD4C5955739F5BEED"
)
SOURCE_RESULT_RECORD_ID = "neo-exp-0043"
...
PROBE_CONFIGURATION = balanced.PrivateBindingConfiguration(
    ...
    domain_separation_identity="ck0-rank-head-v2-joint-condition-intersection-replication-v1",
    protected_starting_sha=STARTING_PROTECTED_MAIN,
)

GEOMETRY_ORDER_DOMAIN = b"ck0-rank-head-v2/joint-condition-intersection-replication/geometry-order-v1\0"
GEOMETRY_COMMITMENT_DOMAIN = b"ck0-rank-head-v2/joint-condition-intersection-replication/geometry-v1\0"
CANDIDATE_COMMITMENT_DOMAIN = b"ck0-rank-head-v2/joint-condition-intersection-replication/candidate-v1\0"
PARENT_COMMITMENT_DOMAIN = b"ck0-rank-head-v2/joint-condition-intersection-replication/parent-v1\0"
EXPERIMENT_KEY_DOMAIN = b"ck0-rank-head-v2/joint-condition-intersection-replication/experiment-key-v1\0"
AUTHORITY_ID_DOMAIN = b"ck0-rank-head-v2/joint-condition-intersection-replication/authority-id-v1\0"
AUTHORITY_HMAC_DOMAIN = b"ck0-rank-head-v2/joint-condition-intersection-replication/authority-hmac-v1\0"
JOURNAL_HMAC_DOMAIN = b"ck0-rank-head-v2/joint-condition-intersection-replication/journal-hmac-v1\0"
AUTHORITY_SCHEMA_VERSION = "rank-head-v2-joint-condition-intersection-replication-authority-v1"
AUTHORITY_RECEIPT_SCHEMA_VERSION = "rank-head-v2-joint-condition-intersection-replication-consumption-v1"
AUTHORITY_KIND = "external-one-shot-joint-condition-intersection-replication"
...
def _require(condition: bool, message: str) -> None:
    if not condition:
        raise JointConditionIntersectionReplicationError(message)
    ...
    *,
    ...
    try:
        if path.is_symlink() or not path.is_file():
            raise JointConditionIntersectionReplicationError(f"{label} is not a regular file")
        data = path.read_bytes()
    except OSError as exc:
        raise JointConditionIntersectionReplicationError(f"{label} is unreadable") from exc
...
def _json_object(path: Path, label: str) -> dict[str, Any]:
    try:
        ...
        raise JointConditionIntersectionReplicationError(f"{label} is malformed") from exc
...
def _load_private(repository: Path) -> tuple[bytes, balanced.PrivateBinding]:
    ...
    private = balanced._private_binding_from_repository(repository, PROBE_CONFIGURATION)
    expected = _probe_binding(root)
    _require(
        private.secret_commitment == expected.secret_commitment
        and private.profile_binding_sha256 == expected.profile_binding_sha256
        and private.alias_map_commitment == expected.alias_map_commitment,
        "private binding reconstruction changed",
    )
    _require(
        isinstance(private.creation_receipt_commitment, str)
        and SHA256_RE.fullmatch(private.creation_receipt_commitment) is not None,
        "private creation receipt commitment changed",
    )
    for previous_relative in PREVIOUS_PRIVATE_ROOT_PATHS:
        previous = repository / previous_relative
        if previous.is_file() and not previous.is_symlink():
            _require(
                _require_regular(previous, "previous private binding root", exact=32) != root,
                "fresh private root duplicates a previous private root",
            )
    return root, private


def prepare_private_binding(repository: Path) -> dict[str, Any]:
    repository = repository.resolve(strict=False)
    for relative in (PRIVATE_ROOT_PATH, PRIVATE_RECEIPT_PATH):
        transaction._require_ignored(repository, repository / relative)
    _require(
        not (repository / PRIVATE_ROOT_PATH).exists()
        and not (repository / PRIVATE_ROOT_PATH).is_symlink()
        and not (repository / PRIVATE_RECEIPT_PATH).exists()
        and not (repository / PRIVATE_RECEIPT_PATH).is_symlink(),
        "fresh private binding already exists",
    )
    balanced.create_private_secret_once(repository, PROBE_CONFIGURATION)
    _root, private = _load_private(repository)
    return {
        "status": "created",
        "profile_id": DESIGN_ID,
        "generated_bytes": 32,
...
def private_geometries(root: bytes, counter: int) -> dict[str, dict[str, Any]]:
    ...
    geometries = {
        "R0": _build_geometry("R0", order[:9]),
        "R1": _build_geometry("R1", order[9:18]),
    }
    _require(
        set(geometries["R0"]["parent-a"] + geometries["R0"]["parent-b"]).isdisjoint(
            geometries["R1"]["parent-a"] + geometries["R1"]["parent-b"]
        ),
        "matched geometries are not identity-disjoint",
    )
    _require(
        geometries["R0"]["unique-intersection"] != geometries["R1"]["unique-intersection"],
...
def build_request(
    ...
    payload = {
        ...
        "response_format": {
            "type": "json_schema",
            "json_schema": {
                "name": "ck0_rank_head_v2_joint_condition_intersection_replication_transform",
                "strict": True,
...
def select_first_eligible(
    ...
    for counter in range(MAXIMUM_SELECTION_ATTEMPTS):
        ...
        if candidate["eligible"]:
            return candidate
    raise JointConditionIntersectionReplicationError(
...
def _source_evidence(repository: Path) -> dict[str, Any]:
    ...
    for line in records:
        try:
            ...
            raise JointConditionIntersectionReplicationError("results ledger is malformed") from exc
    ...
    return {
        ...
        "source_scope": "two-matched-geometries-one-private-binding-one-fixed-seed-transform-only",
    }
...
def build_preregistration_document(repository: Path, model_path: Path) -> dict[str, Any]:
    ...
    controller_binding = _file_binding(
        repository,
        ("scripts/catalytic_kernel_0_balanced_rank_head_v2_joint_condition_intersection_replication.py",),
    )
    document = {
        ...
        "scientific_question": (
            "Does the same commutative unique-intersection-like rank-head operation reproduce "
            "under a jointly new private binding and fixed seed?"
        ),
        "source_evidence": _source_evidence(repository),
        "intervention": (
            "Exactly four transform-only requests over one fresh private binding and one "
            "new commit-derived fixed seed, using two pre-outcome matched geometries each "
            "presented in AB and BA order."
        ),
        "private_binding": {
            "source_root": "fresh-ignored-secure-random-32-byte-root",
            "fresh_private_root_created": True,
            "generation_source": "secrets.token_bytes",
            "generation_count": 1,
            "root_commitment_sha256": private.secret_commitment,
            "profile_binding_sha256": private.profile_binding_sha256,
            "alias_map_commitment_sha256": private.alias_map_commitment,
            "creation_receipt_commitment_sha256": private.creation_receipt_commitment,
            "distinct_from_available_previous_private_roots": True,
        ...
        "request_set": {
            ...
            "restore_requests": 0,
            "fixed_transform_seed": FIXED_TRANSFORM_SEED,
            "fixed_transform_seed_derivation": {
                "domain_hex": FIXED_SEED_DERIVATION_DOMAIN.hex().upper(),
                "material": "utf8(design_id) || 00 || ascii(starting_protected_main)",
                "digest": "SHA-256",
                "integer_law": "1 + uint64_be(digest[0:8]) mod (2^31 - 2)",
                "derived_before_geometry_selection": True,
                "outcome_independent": True,
                "source_seed": SOURCE_FIXED_TRANSFORM_SEED,
                "differs_from_source_seed": True,
            },
        ...
        "decision_law": {
            "joint_condition_replication_supported": (
                "rank zero matches unique intersection in all four requests; R0 and R1 "
                "are each semantically invariant under AB/BA; every preregistered competing "
                "mechanism is false"
            ),
            ...
            "joint_condition_replication_mixed": (
                "some but not all four rank-zero selections match unique intersection"
            ),
            "joint_condition_replication_not_supported": (
                "a coherent competing mechanism explains the outcomes or unique intersection "
                "fails systematically"
            ),
            "inconclusive": (
                "incomplete generations, invalid captures, infrastructure failure, or "
                "inadmissible evidence"
            ),
            "joint_condition_only": True,
            "seed_effect_separately_identified": False,
            "binding_effect_separately_identified": False,
        ...
        "future_live_command_shape": (
            "python scripts/catalytic_kernel_0_balanced_rank_head_v2_joint_condition_intersection_replication.py "
...
def write_preregistration(repository: Path, model_path: Path) -> Path:
    ...
    if path.exists() or path.is_symlink():
        _require(_require_regular(path, "joint-condition-intersection preregistration") == data, "preregistration differs from exact reconstruction")
        return path
    _exclusive_write(path, data)
    return path
...
def validate_preregistration(repository: Path, model_path: Path) -> dict[str, Any]:
    path = repository.resolve(strict=False) / PREREGISTRATION_PATH
    observed_data = _require_regular(path, "joint-condition-intersection preregistration")
    observed = _json_object(path, "joint-condition-intersection preregistration")
...
def consume_authority_once(
    ...
    _require(not path.exists() and not path.is_symlink(), "joint-condition-intersection authority is already consumed")
    body = {
...
def verify_authority_receipt(repository: Path, root: bytes) -> dict[str, Any]:
    receipt = _json_object(
        ...
        "joint-condition-intersection authority receipt",
    )
...
def verify_journal(path: Path, key: bytes) -> list[dict[str, Any]]:
    ...
    for index, line in enumerate(_require_regular(path, "probe journal").splitlines(), start=1):
        try:
            ...
            raise JointConditionIntersectionReplicationError("probe journal is malformed") from exc
...
def _runtime_for_request(
    ...
    spec = transaction.integration.V2RunSpec(
        ...
        source_binding="within-binding-joint-condition-intersection-replication",
        source_profile_id=DESIGN_ID,
        source_full_run_id=request_id,
        authorization_state="external-one-shot-joint-condition-intersection-replication-authority-required",
    )
...
def capture_outcome(
    ...
    try:
        ...
        raise JointConditionIntersectionReplicationError("captured transform is scientifically invalid") from exc
    geometry_id, order = _request_parts(request_id)
...
def adjudicate_outcomes(
    ...
    intersection_match_count = sum(
        bool(outcomes[request_id]["mechanism_matches"]["unique-intersection"])
        for request_id in REQUEST_IDS
    )
    ...
    elif 0 < intersection_match_count < 4:
        classification = "JOINT_CONDITION_REPLICATION_MIXED"
    else:
        classification = "JOINT_CONDITION_REPLICATION_NOT_SUPPORTED"
    result = {
        "classification": classification,
        "mechanism_matches_all_four": all_match,
        "semantic_selection_order_invariant_by_geometry": order_invariance,
        "reproduced_under_combined_new_condition": (
            classification
            == "COMMUTATIVE_UNIQUE_INTERSECTION_LIKE_TRANSFORM_REPRODUCED_UNDER_"
            "JOINTLY_NEW_PRIVATE_BINDING_AND_FIXED_SEED"
        ),
        ...
        "general_transfer_claimed": False,
        "formal_algebra_claimed": False,
        "scope": "jointly-new-private-binding-and-fixed-seed-transform-only-replication",
    }
    ...
    *,
    ...
    if failure is None and set(captures) == set(REQUEST_IDS):
        for request_id in REQUEST_IDS:
            ...
            except JointConditionIntersectionReplicationError as exc:
                failure = {
                    "request_id": request_id,
                    "failure_sha256": sha256_bytes(str(exc).encode("utf-8")),
                    "retry_allowed": False,
...
def main() -> int:
    ...
    try:
        if args.operation == "prepare-private":
            result = prepare_private_binding(repository)
        ...
        JointConditionIntersectionReplicationError,
