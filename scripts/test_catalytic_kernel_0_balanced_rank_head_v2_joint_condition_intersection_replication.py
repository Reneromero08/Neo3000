...
class JointConditionIntersectionReplicationTests(unittest.TestCase):
    @classmethod
    ...
    def test_03_geometries_have_different_shared_candidates_and_decoys(self) -> None:
        g0 = self.geometries["R0"]
        g1 = self.geometries["R1"]
    ...
    def test_07_one_generation_each_and_four_total_are_enforced(self) -> None:
        ...
        with self.assertRaisesRegex(probe.JointConditionIntersectionReplicationError, "duplicate"):
            probe.assert_can_start(started[:1], started[0])
        with self.assertRaisesRegex(probe.JointConditionIntersectionReplicationError, "ceiling"):
    ...
    def test_08_private_geometry_identities_do_not_enter_tracked_artifacts(self) -> None:
        tracked_paths = (
            ...
            REPOSITORY
            / "scripts/catalytic_kernel_0_balanced_rank_head_v2_joint_condition_intersection_replication_scientific.py",
            Path(__file__),
            REPOSITORY / "TASKS.md",
    ...
    def test_10_decision_law_supports_only_the_matching_operation(self) -> None:
        ...
        mixed = json.loads(json.dumps(outcomes))
        mixed[probe.REQUEST_IDS[0]]["mechanism_matches"]["unique-intersection"] = False
        self.assertEqual(
            probe.adjudicate_outcomes(mixed, selections)["classification"],
            "JOINT_CONDITION_REPLICATION_MIXED",
        )

        not_supported = json.loads(json.dumps(outcomes))
        for request_id in probe.REQUEST_IDS:
            not_supported[request_id]["mechanism_matches"]["unique-intersection"] = False
            not_supported[request_id]["mechanism_matches"]["lexical-first"] = True
        self.assertEqual(
            probe.adjudicate_outcomes(not_supported, selections)["classification"],
    ...
    def test_13_fresh_binding_and_seed_are_independent_and_frozen(self) -> None:
        self.assertEqual(
            probe._git(REPOSITORY, "check-ignore", probe.PRIVATE_ROOT_PATH.as_posix()),
            probe.PRIVATE_ROOT_PATH.as_posix(),
        )
        self.assertEqual(
            probe._git(REPOSITORY, "check-ignore", probe.PRIVATE_RECEIPT_PATH.as_posix()),
            probe.PRIVATE_RECEIPT_PATH.as_posix(),
        )
        for relative in probe.PREVIOUS_PRIVATE_ROOT_PATHS:
            previous = REPOSITORY / relative
            if previous.is_file() and not previous.is_symlink():
                self.assertNotEqual(self.root, previous.read_bytes())
        self.assertEqual(probe.FIXED_TRANSFORM_SEED, probe.derive_fixed_transform_seed())
        self.assertNotEqual(
            probe.FIXED_TRANSFORM_SEED, probe.SOURCE_FIXED_TRANSFORM_SEED
        )
        derivation = self.artifact["request_set"]["fixed_transform_seed_derivation"]
        self.assertTrue(derivation["derived_before_geometry_selection"])
        self.assertTrue(derivation["outcome_independent"])
        self.assertTrue(derivation["differs_from_source_seed"])
        self.assertEqual(
            self.private.creation_receipt_commitment,
            self.artifact["private_binding"]["creation_receipt_commitment_sha256"],
        )
