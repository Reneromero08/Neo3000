...
class ChatTokenEvidenceTests(unittest.TestCase):
    ...
    def test_native_ids_without_completion_count_do_not_claim_full_sequence(self) -> None:
        result = self.build(native_token_ids=[1, 2], completion_tokens=None)
        self.assertFalse(result.accepted)
        self.assertFalse(result.full_generated_sequence_known)
        self.assertEqual(result.claim_scope, "none")
