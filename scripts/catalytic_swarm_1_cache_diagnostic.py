...
def classify_diagnostic(
    ...
    if all(item.root_reuse_proven for item in probe_verdicts):
        ...
        reasons.append("both probes hit a stable checkpoint before the public-root boundary")
    ...
    elif any(name == "prompt-prefix-diverged-before-root-end" for name in classes):
        verdict = "reject"
...
def validate_persisted_observation(value: Mapping[str, Any]) -> CacheProbeObservation:
    ...
    return CacheProbeObservation(**{name: value[name] for name in expected})
