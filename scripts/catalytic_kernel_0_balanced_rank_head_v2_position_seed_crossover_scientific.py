#!/usr/bin/env python3
"""Frozen model-contact surface for the position-seed crossover panel.

The controller owns preregistration, authority admission, replay, and
adjudication.  This module owns the last immutable request-hash gate, the
one-generation-per-cell-arm law, raw SSE spooling, and authenticated capture
before any controller parsing.
"""
...
import catalytic_kernel_0_balanced_rank_head_v2_parent_dependence_scientific as parent_scientific


class ScientificSurfaceError(ValueError):
    """The frozen crossover execution surface changed or is unsafe."""


DESIGN_ID = "balanced-opaque-transform-only-within-binding-position-seed-crossover-v1"
PRESENTATIONS = ("P0", "P1")
SEED_BLOCKS = ("S0", "S1")
ARMS = ("full-information", "delete-parent-0", "delete-parent-1")
REQUEST_IDS = tuple(
...
CAPTURE_HMAC_DOMAIN = b"ck0-rank-head-v2/position-seed-crossover/capture-hmac-v1\0"
MAX_CAPTURE_BYTES = 256 * 1024
MAX_RAW_RESPONSE_BYTES = 128 * 1024
CAPTURE_EXECUTION_FIELDS = parent_scientific.CAPTURE_EXECUTION_FIELDS
RawResponseSpool = parent_scientific.RawResponseSpool
...
_CONTRACT_FIELDS = {
    ...
    "presentation_proof_commitments",
    "seeds",
    "model_sha256",
    "binary_sha256",
    "carrier_root_sha256",
...
def capture_schema() -> dict[str, Any]:
    return {
        ...
        "authentication": "HMAC-SHA-256 under the private panel experiment key",
...
def frozen_scientific_binding(
    ...
    *,
    ...
    """Bind the immutable request set and exact contact implementation."""
    ...
    execution_order = tuple(contract.get("execution_order", ()))
    _require(
        len(execution_order) == len(REQUEST_IDS)
        and set(execution_order) == set(REQUEST_IDS),
    ...
    body = {
        "surface_version": 1,
        "module": {
            "path": "scripts/catalytic_kernel_0_balanced_rank_head_v2_position_seed_crossover_scientific.py",
        ...
        "one_request_started_maximum_per_cell_arm": True,
        "one_generation_maximum_per_cell_arm": True,
        "twelve_generations_maximum_overall": True,
...
def verify_capture(
    ...
    *,
    ...
    _require(set(body) == set(capture_schema()["required_fields"]), "response capture field set changed")
    _require(
...
def execute_and_capture_request(
    *,
    ...
    """Issue one cell-arm generation and capture it before controller parsing."""
    ...
    append_event(
        ...
        facts={
            ...
            "maximum_generations_for_cell_arm": 1,
...
def request_set_is_exact(values: Sequence[str]) -> bool:
    return len(values) == len(REQUEST_IDS) and set(values) == set(REQUEST_IDS)
