...
def parse_netstat_listener_pids(output: str, port: int) -> set[int]:
    ...
    for raw_line in output.splitlines():
        ...
        if len(parts) < 4 or parts[0].upper() != "TCP":
            continue
        protocol, local_endpoint, _foreign_endpoint, state = parts[:4]
        if protocol.upper() != "TCP" or state.upper() != "LISTENING":
            continue
        if _endpoint_port(local_endpoint) != port:
            continue
        if len(parts) < 5:
            raise ValueError(f"missing listener PID for port {port}")
        pid_text = parts[4]
...
def query_listener_pids(
    ...
    *,
    ...
    """Return the first available checked PID set within one hard retry window."""
    _validate_port(port)
    if max_attempts <= 0:
    ...
    while attempts < max_attempts:
        ...
        if remaining <= 0:
            break
        attempts += 1
        sample = sample_fn(port, min(timeout_seconds, remaining))
        backend = sample.backend
        latencies.append(sample.elapsed_seconds)
        actual = sample.pids
        if sample.available:
            return ListenerQueryEvidence(
                True,
                port,
                sample.pids,
                backend,
...
def qualify_listener_ownership(
    ...
    *,
    ...
    if any(not isinstance(pid, int) or isinstance(pid, bool) or pid <= 0 for pid in expected):
        raise ValueError("expected listener PID set must contain only positive integers")
    if max_attempts <= 0:
        raise ValueError("max_attempts must be positive")
    if timeout_seconds <= 0 or max_window_seconds <= 0:
        raise ValueError("listener retry timeouts must be positive")
    ...
    hard_mismatch = query.passed and query.pids != expected
    return ListenerOwnershipEvidence(
        query.passed and not hard_mismatch,
        hard_mismatch,
        port,
        expected,
        query.pids,
        query.backend,
        query.attempt_count,
        query.successful_sample_count,
        query.timeout_count,
        query.unavailable_count,
        query.latencies_seconds,
        query.errors,
        "listener-pid-mismatch" if hard_mismatch else query.final_error,
    )
