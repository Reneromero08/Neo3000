...
from chat_stream_terminal_evidence import TerminalStopEvidence, terminal_eos_gate
...
def resolve_fast_token_evidence(
    ...
    *,
    ...
    raw_terminal = _read(measurement, "terminal_stop_evidence")
    terminal: TerminalStopEvidence | None = None
    if raw_terminal is not None:
        if not isinstance(raw_terminal, dict):
            raise FastTokenEvidenceError("terminal stop evidence is not an object")
        try:
            terminal = TerminalStopEvidence(**raw_terminal)
        except (TypeError, ValueError) as exc:
            raise FastTokenEvidenceError("terminal stop evidence is malformed") from exc
        if _read(measurement, "stop_type") != terminal.stop_type:
            raise FastTokenEvidenceError("terminal stop type contradicts the stream measurement")
        if _read(measurement, "stopping_word") != terminal.stopping_word:
            raise FastTokenEvidenceError("terminal stopping word contradicts the stream measurement")
    terminal_gate = terminal_eos_gate(terminal)

    tokenizer_repeat: dict[str, Any] = {
        "performed": False,
        "equal": None,
        "first": None,
        "second": None,
    ...
    def deterministic_tokenize(text: str) -> list[int]:
        first = tokenize_visible_content(text)
        second = tokenize_visible_content(text)
        tokenizer_repeat.update({
            "performed": True,
            "equal": first == second,
            "first": first,
            "second": second,
        ...
        if first != second:
            raise ChatTokenEvidenceError("tokenizer reconstruction was not deterministic")
        return first
    ...
    try:
        evidence = build_chat_token_evidence(
            ...
            tokenize_visible_content=deterministic_tokenize,
            finish_reason=_read(measurement, "finish_reason"),
            terminal_stop_type=terminal.stop_type if terminal is not None else None,
            terminal_stopping_word=terminal.stopping_word if terminal is not None else None,
            stop_sequences_configured=stop_sequences_configured,
            allow_terminal_control_accounting=(
                allow_terminal_control_accounting and terminal_gate["passed"] is True
            ),
        )
    ...
    result["terminal_eos_gate"] = terminal_gate
    result["tokenizer_repeat"] = tokenizer_repeat
    return result
