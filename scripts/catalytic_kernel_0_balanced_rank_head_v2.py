...
def _require_exact_implementation_paths(implementation_paths: Sequence[str]) -> None:
    if any(not isinstance(path, str) for path in implementation_paths):
...
def build_preregistration_document(
    *,
    ...
    if audits != REQUIRED_AUDITS:
        raise RankHeadDesignError("v2 audit names and outcomes must be exact terminal PASS")
    verification = dict(static_verification or {})
    if verification.get("status") != "pass":
        raise RankHeadDesignError("v2 static verification must be terminal PASS")
    document = {
        ...
        "execution_state": dict(EXPECTED_EXECUTION_STATE),
    }
...
def validate_preregistration(
    ...
    *,
    ...
    if not isinstance(source_binding, Mapping):
        raise RankHeadDesignError("v2 implementation binding is not an object")
    files = source_binding.get("files", [])
    if not isinstance(files, list) or any(not isinstance(item, Mapping) for item in files):
        raise RankHeadDesignError("v2 implementation binding file set is malformed")
    paths = [item.get("path") for item in files]
    _require_exact_implementation_paths(paths)
    ...
    if document.get("execution_state") != EXPECTED_EXECUTION_STATE:
