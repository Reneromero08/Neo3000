...
def extract_terminal_stop_evidence(
    ...
    *,
    ...
    if "__verbose" not in event:
        return None
    verbose = event.get("__verbose")
    if not isinstance(verbose, dict):
        raise TerminalEvidenceError("verbose terminal evidence is not an object")
    if not any(key in verbose for key in ("stop", "stop_type", "stopping_word")):
        raise TerminalEvidenceError("verbose terminal evidence omits stop metadata")
