...
def _run_rank_head_v2_protected(
    ...
    *,
    ...
    try:
        if caller is None or caller.f_code is not entrypoint.run_rank_head_v2.__code__:
            raise RankHeadV2LiveError(
                "protected live core is callable only by the authoritative "
    ...
    authority.validate_external_authority(
        ...
        expected_model_sha256=str(public_preflight["model_identity"]["sha256"]),
        expected_binary_sha256=str(public_preflight["binary_identity"]["sha256"]),
    )
