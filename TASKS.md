...
**Next exact action:** `PREREGISTER_NEO_EXP_0065_STRICT_PREFIX_PLUS_PROCESS_LOCAL_NGRAM_CACHE_N1_LATENCY` — change only launch speculative type `none` to `ngram-cache` on the same pinned candidate and exact 689-token root. Preserve exact utility/root/lifecycle controls, record draft acceptance and second-carrier residency, charge the learning warmup, compare against canonical 0064, and require sidecar-retirement closure; make no live contact during preregistration.
...
- [ ] `PREREGISTER_NEO_EXP_0065_STRICT_PREFIX_PLUS_PROCESS_LOCAL_NGRAM_CACHE_N1_LATENCY`. Add the smallest scoped launch-argument seam and dedicated no-contact controller/test package. Change only speculative type `none` to `ngram-cache`; preserve checkpoint zero, strict root `689`, submitted prompt `690`, exact `689/1`, branch/input/output identities, T=4 geometry, direct-cache control, root closure, and stable custody. Record draft/accepted counts and n-gram residency, charge learning warmup, label launch-global cache-disabled routes `ngram-only` rather than speculation-free, require improvement over canonical 0064, and close the second carrier by process retirement.
