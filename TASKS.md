...
**Active bounded objective:** Preserve accepted `neo-exp-0046` as valid worker-to-synthesis capability evidence while treating its current DSL carrier as capability-only, not a fresh-inference-advantage test. The controller can deterministically reconstruct both worker relations, their singleton intersection, and the protected score with zero model generations. No suitable non-controller-reconstructible worker-synthesis task family currently exists in the repository; all broader claims remain locked.
**Next exact action:** `STATIC_DESIGN_OF_ONE_EASY_35B_CALIBRATED_NON_CONTROLLER_RECONSTRUCTIBLE_WORKER_SYNTHESIS_TASK_FAMILY`. Define only the smallest task family, protected evaluator, direct baseline, and resource-accounting boundary. Keep evidence shards short, the structured schema small, synthesis to one simple relation, and output ceilings bounded; do not implement it, create authority, or run it without separate authorization.
...
- [ ] Separately authorize static design only for one easy Agents-A1 35B-calibrated non-controller-reconstructible worker-synthesis task family. Use short evidence shards, a small structured schema, one simple synthesis relation, and bounded output ceilings. Require individually insufficient model-generated partials, a protected utility evaluator, a same-evidence direct baseline, hidden-independent route selection, and exact resource accounting. Do not generate a corpus, implement runtime, or execute inference automatically.
