...
**Next exact action:** `PREREGISTER_NEO_EXP_0066_STRICT_PREFIX_PLUS_PARTIAL_GPU_MOE_N1_LATENCY` — preserve the accepted strict 689-token RAM root and change only MoE placement from all experts on CPU to a bounded first-N-layers-on-CPU profile. Admit the candidate only below the existing 6,000 MiB WDDM ceiling; keep checkpoint zero, exact output, N=1, no speculation, no fanout, and canonical neo-exp-0064 as the strict comparator.
...
- [ ] `PREREGISTER_NEO_EXP_0066_STRICT_PREFIX_PLUS_PARTIAL_GPU_MOE_N1_LATENCY`. Replace only all-CPU expert placement with one exact `--n-cpu-moe N` profile chosen from the measured 2,725 MiB / 19.1 TPS and 10,604 MiB / 30.8 TPS endpoints. Preserve no speculation, strict root `689`, submitted prompt `690`, exact `689/1`, branch/input/output identities, T=4 geometry, complete lifecycle, isolated sidecar, stable custody, and N=1/no-fanout scope. Fail before scientific contact unless readiness proves exact launch identity and WDDM below 6,000 MiB.
