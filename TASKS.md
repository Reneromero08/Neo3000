...
**Latest causal boundary:** `neo-exp-0062` at `lab/results.jsonl:74`, SHA-256 `21F11CB8A35BFF70261EA0023B7583FD06DFE3B5DA49B246A4CE87F9FB7674BD`; checkpoint-free N=1 temporal root reuse on exact water branch 7 passed all identity, utility, lifecycle, residency, and latency gates across four counted pairs. Median restore-inclusive effective wall was `2.913` seconds catalytic versus `10.571` direct (`3.629x`); the counted four-request full lifecycle remained positive at `1.672x` wall and `1.921x` fresh compute.
...
**Identity status:** `neo-exp-0062` is scientifically consumed by its accepted bounded latency result. `neo-exp-0063` is reserved and statically preregistered for one full-prompt-root N=1 latency execution; no live successor contact has occurred.
...
**Next exact action:** `EXECUTE_NEO_EXP_0063_FULL_PROMPT_ROOT_N1_LATENCY` — from the exact clean preregistration head, invoke the dedicated full-prompt wrapper once at checkpoint zero with pinned identities and distinct absent artifacts; adjudicate only the frozen 690-root, 689/1 route split, cross-run improvement, matched-direct drift, utility, and full-lifecycle gates.
...
- [ ] `EXECUTE_NEO_EXP_0063_FULL_PROMPT_ROOT_N1_LATENCY`. Preserve water branch 7, exact 690-token input hash, seed/sampler, checkpoint zero, one warmup CD and four counted CD/DC/DC/CD pairs, direct 0/690 control, complete lifecycle accounting, closure, residency, and no-fanout scope. Require saved root 690 and catalytic 689/1, exact identical utility/hashes, at least `3/4` paired wins, every direct-relative speed gate at least `1.25x`, prompt/TTFT/effective-wall medians each at least `1.25x` better than 0062's 543-root medians, matched-direct drift at most `10%`, and positive counted lifecycle wall/fresh advantage.
