...
**Next exact action:** `PREPARE_BOUNDED_STARTUP_HEALTH_RECOVERY_SUCCESSOR_FOR_WATER_DIRECT_N16` — use a new experiment ID; change only startup stable-health handling to the existing bounded 15-second/three-success recovery; preserve the exact water panel and scientific controls; statically test, audit, ledger, and commit it; then stop before live stable-server contact unless separately authorized under the active `/goal` pause gate.
...
- [ ] Under a new experiment ID, expose only the already tested startup-only stable-health recovery policy (`15` second maximum failure window; `3` consecutive successes) to the unchanged water controller. Prove no post-readiness recovery, no scientific-control drift, and no model/server contact during static preparation; ledger and commit the exact successor, then pause before live contact as required by `/goal`.
