...
**Next exact action:** `PREREGISTER_NEO_EXP_0067_SAME_BINARY_PAIRED_HOST_VS_DEVICE_ROOT` — use the exact current nine-module runtime for one host-root arm and one device-root arm. Give each launch one identical pre-root direct sentinel, then run only the catalytic measurements needed to compare restore, prompt, generation, remainder, and whole wall. This removes redundant per-pair direct controls while isolating carrier effect from ambient drift; do not retry `neo-exp-0066`.
...
- [ ] `PREREGISTER_NEO_EXP_0067_SAME_BINARY_PAIRED_HOST_VS_DEVICE_ROOT`. Use the exact current nine-module runtime for both arms; change only root storage. Preserve strict `689/1`, branch 7, checkpoint zero, N=1/T=4, no speculation/fanout, output/lifecycle/resource/cleanup gates, and one pre-root direct sentinel per launch. Remove the four redundant per-pair direct routes from each arm and compare normalized restore, prompt, generation, remainder, and whole-wall deltas.
