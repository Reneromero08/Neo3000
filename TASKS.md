...
**Active bounded objective:** Binding-1 r2 is consumed and retired as `SUCCESS_REPORTED_EVIDENCE_CUSTODY_LOST_AFTER_TEST_OVERWRITE`. Its unchanged receipt and manifest plus the overwritten result and closure are preserved as forensic evidence, but r2 has no scientific publication record and cannot unlock binding-2. The state-dependent tests are isolated, terminal evidence is non-overwritable and byte-exactly archived, and fresh binding-1 run `ck0-balanced-v2-rank-head-b1-full-r3` is the only next run.
**Next exact action:** `Separately authorize only ck0-balanced-v2-rank-head-b1-full-r3 with a fresh external authority ID bound to the exact hardened protected commit.` Binding-2 remains `ck0-balanced-v2-rank-head-b2-full-r1`, unexecuted and unauthorized until r3 is visibly published. There is no automatic follow-on. End-to-end cross-binding replication, binding-2 parent dependence, causal replication across bindings, general two-parent necessity, transfer, general catalytic inference, task advantage, superiority, SOTA, broader/restart HoloState, Deep, and automatic promotion remain locked.
...
- [ ] Separately authorize only `ck0-balanced-v2-rank-head-b1-full-r3` with a fresh external authority ID bound to the exact hardened protected commit. Permit one invocation and no retry. Do not create binding-2 authority or auto-follow.
