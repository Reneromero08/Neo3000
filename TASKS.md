...
**Active bounded objective:** Preserve published `neo-exp-0045`, both cardinality-three corpus diagnostics, and the statically prepared cardinality-five bilateral worker-relation probe. The frozen task-03 profile has shards `012` and `234`, two five-member locally exact supports, one shared candidate, and a protected hidden-exactness gate of `16/16`; no live authority or runtime state exists.
**Next exact action:** `SEPARATELY_AUTHORIZE_ONE_LIVE_BALANCED_OPAQUE_FIVE_SUPPORT_TWO_WORKER_SYNTHESIS_PROBE`. Bind a fresh external one-shot authority to the final published static commit and exact preregistration. Do not execute automatically. Worker synthesis, a complete catalytic lifecycle, task advantage, reduced fresh computation, compute amplification, general catalytic inference, SOTA, and automatic promotion remain locked.
...
- [ ] Separately authorize one live `balanced-opaque-five-support-two-worker-synthesis-probe-v1` execution from the final published static commit with a fresh external one-shot authority. Do not execute automatically or add another task, seed, binding, baseline, control, or follow-on.
