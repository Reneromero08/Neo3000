...
**Latest causal boundary:** `neo-exp-0061` at `lab/results.jsonl:73`, SHA-256 `AA2BF8B86A51C1C79C7A5C688C765E7D9847133C7261EB48169F5DC05EA88F28`; the frozen water direct-only checkpoint-zero panel passed exact `16/16` with sequence `BACDABCDABCDABCD`, zero cached prompt tokens, and zero carrier/root/snapshot/cache-enabled paths. Together with cold-chain, two independent exact direct controls now exist; datacenter remains rejected at `15/16`.
...
**Next exact action:** `EXECUTE_NEO_EXP_0062_SINGLE_REQUEST_CATALYTIC_LATENCY` — from the exact clean preregistration head, execute the pinned N=1 water branch-7 controller once with checkpoint zero, distinct absent result/log paths, and the existing isolated-sidecar safety controls; adjudicate only the frozen latency and lifecycle gates.
...
Current evidence remains bounded: exact process-local reuse and equal-utility fanout amortization are supported through N=8; N=16 positive amortization was observed but its strict utility gate failed; direct-only controls accepted cold-chain and water at `16/16` while datacenter remains rejected at `15/16`. Per user direction, the active research cursor now prioritizes fast single-request catalytic inference rather than further fanout scaling.
...
- [ ] `EXECUTE_NEO_EXP_0062_SINGLE_REQUEST_CATALYTIC_LATENCY`. Commit the static package, verify the exact clean head plus pinned candidate/model identities and isolated ports, then invoke the preregistered controller once. Preserve every warmup and counted route, root operation, timing identity, resource sample, teardown fact, result/log artifact, and terminal verdict; append `lab/results.jsonl` only from executed evidence.
