...
**Active bounded objective:** Preserve the published `neo-exp-0043` result and the frozen hard out-of-sample replication design `balanced-opaque-transform-only-joint-condition-intersection-replication-v1`. The successor changes one private binding and one fixed seed jointly, so it can test reproduction under the combined condition but cannot separately identify seed or binding effects.
**Next exact action:** Separately authorize exactly one live invocation of the frozen four-request replication from its published static commit using a fresh external authority ID. Do not create authority or execute automatically. Formal algebra, seed invariance, binding invariance, transfer, general catalytic inference, task advantage, superiority, SOTA, and automatic promotion remain locked.
...
- [ ] Separately authorize exactly one live invocation of the frozen joint-condition replication with a fresh external authority ID bound to the published static commit. Do not execute automatically and do not infer separate seed or binding invariance from this combined condition.
