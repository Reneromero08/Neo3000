...
**Active bounded objective:** Preserve and publish the completed within-binding position-seed crossover. The exact 12-cell execution supports a singleton presentation-position effect and shows that the complete parent pair overcomes the unfavorable singleton-last condition across both fixed seeds. The earlier Parent-A-only directional explanation is retained as historical evidence but retired as a stable mechanism because presentation position confounded it. This is a transform-causal-core result under one matched private binding, not a complete new borrow -> transform -> extract -> restore cycle.
**Next exact action:** Separately authorize one bounded static scientific design choice for the next smallest experiment that tests whether this position-conditional effect generalizes beyond the matched binding or complete transform-core panel. Do not authorize execution automatically. Position-independent bilateral dependence, general two-parent necessity, cross-binding generalization of this crossover, transfer, general catalytic inference, task advantage, superiority, SOTA, broader/restart HoloState, Deep, and automatic promotion remain locked.
...
- [ ] Separately authorize one bounded static scientific design choice for the next smallest discriminating experiment. Do not create authority or execute a follow-on automatically.
