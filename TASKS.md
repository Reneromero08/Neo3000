...
**Identity status:** `neo-exp-0062` is scientifically consumed by its accepted bounded latency result. No live successor contact has occurred; the active static cursor is causal localization of its remaining single-request prompt hot path before reserving `neo-exp-0063`.
...
**Next exact action:** `PROFILE_AND_CAUSALLY_LOCALIZE_THE_DOMINANT_REMAINING_SINGLE_REQUEST_HOT_PATH_WITH_THE_SAME_EXACT_CONTROL` — use the executed timing decomposition and a zero-contact source/controller audit to freeze the narrowest one-variable `neo-exp-0063`; prioritize the remaining approximately `2.5` seconds of fresh prompt prefill, not fanout.
...
- [ ] `PROFILE_AND_CAUSALLY_LOCALIZE_THE_DOMINANT_REMAINING_SINGLE_REQUEST_HOT_PATH_WITH_THE_SAME_EXACT_CONTROL`. Preserve the exact water branch, checkpoint zero, token array, seed, sampler, T=4 ABBA schedule, and lifecycle accounting. Determine whether extending only the prompt root from `543` to the full `690` submitted prompt is the narrowest lawful intervention to remove the observed catalytic median `2498.008 ms` prompt-processing remainder before any C++ runtime change.
