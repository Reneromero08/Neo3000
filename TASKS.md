...
**Active bounded objective:** Preserve published `neo-exp-0045` and the public-only worker-geometry diagnostic `EXISTING_CORPUS_THREE_WORKER_GEOMETRY_UNAVAILABLE`. The frozen eight-task corpus contains at most two locally eligible worker shards for any task, so the required three-worker profile does not exist without changing the corpus or gates.
**Next exact action:** `SEPARATELY_AUTHORIZE_A_STATIC_SCIENTIFIC_DECISION_ON_WORKER_GEOMETRY_CORPUS_COMPATIBILITY`. Do not create a synthetic task, weaken the frozen gates, build a live controller, or execute automatically. Worker synthesis, a complete catalytic lifecycle, task advantage, reduced fresh computation, compute amplification, general catalytic inference, SOTA, and automatic promotion remain locked.
...
- [ ] Separately decide whether the frozen corpus/task geometry should be revised for a worker-synthesis capability probe or whether this line should stop. Do not create a synthetic task or weaken the current structural gates without explicit authorization.
