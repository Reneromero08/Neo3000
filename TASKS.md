...
**Active bounded objective:** Preserve accepted `neo-exp-0046` as capability evidence and preserve the current DSL carrier as capability-only. The easy Agents-A1 35B-calibrated `two-shard-semantic-xor-worker-synthesis-family-v1` now has a frozen four-task public corpus and separately bound ignored evaluator with exact aggregate `00 / 01 / 10 / 11` coverage. No implementation, prompt, seed, authority, model contact, inference, or result exists for this family; all broader claims remain locked.
**Next exact action:** `SEPARATELY_IMPLEMENT_AND_PREREGISTER_TWO_SHARD_SEMANTIC_XOR_WORKER_AND_BASELINE_EVALUATION`. Implement only the frozen worker, XOR synthesis, direct baseline, capture, protected scoring, and exact resource-accounting surfaces. Create no authority or live execution without later separate authorization.
...
- [ ] Separately implement and statically preregister the frozen two-shard semantic-XOR worker and direct-baseline evaluation. Preserve exact public/evaluator bytes, use no protected answer for rendering or route decisions, and stop before authority or live execution.
