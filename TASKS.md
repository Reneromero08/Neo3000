...
**Latest causal boundary:** `neo-exp-0067` at `lab/results.jsonl:79`, SHA-256 `B2F879858BD3C1D0ACCC79057D826F3082B26D8109EA80A3A1C3E43AF095EBC4`; the same-runtime host control was exact and complete. CUDA device storage improved internal restore `7.315x`, client-visible restore `3.032x`, and TTFT `1.093x`, but raw restore-inclusive wall was `0.985x` (`0.5395` versus `0.5315 s`), so whole-request speedup is not established. Direct-normalized wall was descriptively `1.125x`, but has no claim authority.
**Latest resolved engineering incident:** the earlier pre-`Popen` runtime-identity-lock failure is preserved in `frontier-attempt-0069`; bounded controlled-path delegation was validated and committed at `89644ea61de7a200058030e02d9f55d8f319d3a9`, after which the same unconsumed scientific identity executed successfully with exact binary/model custody.
**Identity status:** `neo-exp-0063` through `neo-exp-0067` are scientifically consumed and cannot be retried under those identities. `neo-exp-0067` qualifies the same-runtime control while preserving CUDA-root whole-wall improvement as unsupported.
**Claim ceiling:** `PROCESS_LOCAL_RUNTIME_NATIVE_CARRIER_FANOUT_AMORTIZATION_SUPPORTED_TO_N8 / CHECKPOINT_FREE_PROCESS_LOCAL_N1_TEMPORAL_ROOT_REUSE_FAST_SINGLE_REQUEST_LATENCY_SUPPORTED_ON_WATER_BRANCH_7_AT_T4`
**Mechanism status:** `EXACT_PROCESS_LOCAL_HOLOSTATE_REUSE_PROVEN / BOUNDED_CATALYTIC_FANOUT_ADVANTAGE_REPLICATED / CHECKPOINT_ENABLED_EXECUTION_PATH_CAUSAL_AT_TICK11 / CHECKPOINT_FREE_NATIVE_RAM_ROOT_LIFECYCLE_EXACT_AT_TICK11 / CHECKPOINT_FREE_N16_FIXED_RESIDENCY_AND_POSITIVE_AMORTIZATION / WEAK_DIRECT_BASELINE_REJECTS_EQUAL_UTILITY / COLDCHAIN_STRONG_DIRECT_N16_QUALIFIED / DATACENTER_STRONG_DIRECT_N16_REJECTED_15_OF_16 / WATER_STRONG_DIRECT_N16_QUALIFIED / TWO_INDEPENDENT_EXACT_DIRECT_CONTROLS_ESTABLISHED / BOUNDED_FAST_SINGLE_REQUEST_CATALYTIC_LATENCY_SUPPORTED_AT_N1_T4 / EXACT_FULL_PROMPT_ROOT_REJECTED_BY_HYBRID_RECURRENT_FULL_REPROCESSING / STRICT_689_TOKEN_PREFIX_ROOT_ACCEPTED_WITH_689_OF_690_PROMPT_TOKENS_REUSED / RESIDUAL_GENERATION_PHASE_DOMINANT_AT_49_PERCENT / PROCESS_LOCAL_NGRAM_CACHE_REJECTED_BY_DRAFT_VERIFICATION_OVERHEAD_SIGNAL / CUDA_GPU_RESIDENT_ROOT_EXACT_AND_7_315X_FASTER_INTERNAL_RESTORE / CUDA_ROOT_TTFT_1_093X_FASTER / CUDA_ROOT_WHOLE_WALL_SPEEDUP_NOT_SUPPORTED / BOUNDED_AUTONOMOUS_SUCCESSOR_MODE_ACTIVE`
...
**Next exact action:** `EXECUTE_NEO_EXP_0068_CUDA_ROOT_PLUS_PARTIAL_GPU_MOE_N1_LATENCY` — execute once from the exact clean preregistration head with device-root storage plus exact `--n-cpu-moe 26`. Readiness must provide a current exact-PID WDDM sample below 6,000 MiB before Task-A or any model request.
...
Current evidence remains bounded: exact process-local reuse and equal-utility fanout amortization are supported through N=8; N=16 positive amortization was observed but its strict utility gate failed; direct-only controls accepted cold-chain and water at `16/16` while datacenter remains rejected at `15/16`; and the checkpoint-free 689-token strict-prefix water-branch root supports exact `689/1` prompt reuse with `0.4610`-second median restore-inclusive wall versus `10.7735` seconds direct through four counted pairs. Per user direction, the active research cursor continues reducing residual single-request catalytic latency rather than scaling fanout.
...
**Frontier friction law:** retain only exact identity, one-variable causality, output/utility, bounded resources, isolation/cleanup, and one compact durable result. Do not add per-run permission pauses, duplicate reviews, report farms, or historical timing gates when a same-cycle control can answer the question. Batch board/checkpoint/ledger synchronization into the experiment handoff and continue automatically.
...
- [ ] `EXECUTE_NEO_EXP_0068_CUDA_ROOT_PLUS_PARTIAL_GPU_MOE_N1_LATENCY`. Execute once with pinned nine-module runtime/model, checkpoint zero, strict `689/1`, exact branch 7, T=4, no speculation/fanout, distinct absent artifacts, readiness WDDM admission, and no retry after the first model-facing request.
