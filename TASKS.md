...
**Identity status:** `neo-exp-0061` remains the active, unconsumed scientific experiment. The failed pre-`Popen` controller attempt is an engineering incident, not a scientific execution.
...
**Next exact action:** `REPAIR_VALIDATE_AND_EXECUTE_NEO_EXP_0061_CONTROLLED_RUNTIME_IDENTITY_LOCK_BINDING` — repair only the discovery-sidecar controlled-readiness identity path-lock binding while preserving experimental-binary acceptance and every water-panel scientific control; validate it, execute the still-active preregistered `neo-exp-0061` under standing frontier authority, adjudicate and record the result, advance the cursor, and continue the autonomous research loop.
...
- [ ] `REPAIR_VALIDATE_AND_EXECUTE_NEO_EXP_0061_CONTROLLED_RUNTIME_IDENTITY_LOCK_BINDING`. Make the one infrastructure-only correction so the discovery sidecar retains its experimental-binary identity checks while invoking the base controlled-readiness path-lock acquisition and revalidation. Add focused tests, preserve every preregistered water-panel scientific control, execute the still-active preregistered `neo-exp-0061` under standing authority, adjudicate it, and continue to the next lawful cursor without a routine handoff.
