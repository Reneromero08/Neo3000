...
**Latest causal boundary:** `neo-exp-0066` at `lab/results.jsonl:78`, SHA-256 `A039B9D293197C8412921A22C0B31418A339083DB63AADB1C3CC6E8572BBCABE`; non-host CUDA RAM-root storage preserved exact `689/1` utility and closure while improving internal restore from `13.9525` to `2.2510 ms` (`6.198x`), but its `0.5395`-second restore-inclusive wall missed the historical host-root gate at `0.854x`. Same-run direct controls drifted about `33.5%`, so a contemporaneous host-mode control is required before attributing the whole-wall difference.
...
**Identity status:** `neo-exp-0063`, `neo-exp-0064`, `neo-exp-0065`, and `neo-exp-0066` are scientifically consumed and cannot be retried under those identities. `neo-exp-0066` is preserved as an exact positive CUDA-restore result and a negative preregistered whole-wall result, not retried or promoted.
...
**Next exact action:** `EXECUTE_NEO_EXP_0067_SAME_RUNTIME_HOST_ROOT_CONTROL` — execute one host-root arm with the exact nine-module CUDA-capable runtime used by consumed device-root `neo-exp-0066`. Reuse 0066 as the device arm rather than rerunning it; preserve the complete strict-prefix geometry so same-binary host/device restore, prompt, generation, remainder, direct drift, and whole wall are directly comparable.
...
- [ ] `EXECUTE_NEO_EXP_0067_SAME_RUNTIME_HOST_ROOT_CONTROL`. Execute once from the exact clean preregistration head with distinct absent artifacts and isolated 9494. Compare its host restore, prompt, generation, request remainder, direct controls, and whole wall against consumed same-runtime device evidence from 0066; do not rerun either identity.
