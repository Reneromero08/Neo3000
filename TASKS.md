...
**Active bounded objective:** Preserve published semantic-XOR Attempt-3 adjudication exactly and keep `holostate-v1-warm-trajectory-related-task-evaluation-v1` frozen while repairing only its protected-evaluator delayed-access boundary. Precontact custody is metadata-only; exact evaluator bytes remain unopened, unhashed, and unparsed until all 12 captures, cleanup, and postflight pass.
**Next exact action:** `INDEPENDENT_STATIC_AUDIT_WARM_TRAJECTORY_EVALUATOR_ISOLATION`, followed only on acceptance by `AUTHORIZE_ONE_LIVE_HOLOSTATE_WARM_TRAJECTORY_RELATED_TASK_EVALUATION` with one fresh external authority bound to the final pushed repair commit.
...
- [x] Freeze the direct catalytic decision law: require both routes to complete all four pairs, catalytic accuracy at least direct accuracy, and strictly fewer fresh Task-B prompt-plus-completion tokens per correct Task-B answer by exact integer cross-products. Keep general catalytic inference, transfer, restart persistence, superiority, SOTA, and promotion locked.
- [x] Repair only protected-evaluator isolation. Precontact custody now uses filesystem metadata plus ignored/untracked Git state and reports bytes unopened/unhashed/unparsed; terminal scoring performs one exact read, size/hash verification, duplicate-key-rejecting JSON parse, and boolean-only scoring after all gates. Regenerated preregistration artifact/file are `68F2447D47BC912CF171AA4E7DF5CD7552016EC9992C2FD1D2E50339B07BE6A3` / `5DDAC3E162A6B1A43292347B43B5DADA9ADE5A7D0C44598BDC1E6CB97DD4617F`; protected scorer is `0B5B079DDA160C535722967E2EBA7F60991AA522B24AD055A18D14FF698B92B7`.
- [ ] `INDEPENDENT_STATIC_AUDIT_WARM_TRAJECTORY_EVALUATOR_ISOLATION`. Confirm no evaluator byte access before terminal scoring and no scientific surface drift.
