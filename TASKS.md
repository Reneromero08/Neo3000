...
**Next exact action:** `PREREGISTER_NEO_EXP_0068_CUDA_ROOT_PLUS_PARTIAL_GPU_MOE_N1_LATENCY` — preserve the proven device-root restore path and change only expert placement from all 40 MoE layers on CPU to the first 26 on CPU / last 14 on GPU. The conservative endpoint interpolation predicts about 5,483 MiB WDDM, below the 6,000 MiB ceiling, while targeting the dominant six-token generation phase.
...
- [ ] `PREREGISTER_NEO_EXP_0068_CUDA_ROOT_PLUS_PARTIAL_GPU_MOE_N1_LATENCY`. Preserve device root and change only base MoE launch placement from `--cpu-moe` to exact `--n-cpu-moe 26`, leaving 14/40 expert layers on GPU. Require readiness WDDM below 6,000 MiB before model requests, exact output/root closure, and improvement over consumed device-root 0066 in generation and restore-inclusive wall.
