...
**Active bounded objective:** Preserve the published inherited Task-A carrier result and statically select the minimum response to its now-confirmed executable-reuse boundary. Preserve the immutable live exact-reuse failure and every broader claim lock.
**Next exact action:** `STATICALLY_SELECT_MINIMUM_RESPONSE_TO_EXECUTABLE_REUSE_BOUNDARY` — choose, but do not execute, the smallest response to the confirmed 4-token template divergence plus 128-token safe-checkpoint rollback.
