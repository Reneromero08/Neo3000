...
**Latest causal boundary:** `neo-exp-0065` at `lab/results.jsonl:77`, SHA-256 `42128B9BE51C144B7D6F9639205058B479CF690763448EB3C990F4CE7201F67B`; process-local `ngram-cache` accepted one of eight drafts on every counted catalytic request and preserved exact `689/1` output, but increased median generation from `225.1865` to `511.1520 ms` and restore-inclusive wall from `0.4610` to `0.7935` seconds. The strict speed gate rejected it despite exact closure and a barely positive `1.001x` charged lifecycle versus canonical direct.
...
**Identity status:** `neo-exp-0063`, `neo-exp-0064`, and `neo-exp-0065` are scientifically consumed and cannot be retried under those identities. `neo-exp-0065` is preserved as an exact negative latency result, not an engineering failure.
...
**Next exact action:** `EXECUTE_NEO_EXP_0066_STRICT_PREFIX_CUDA_RESIDENT_RAM_ROOT_N1_LATENCY` — execute the independently cleared device-root package once from its exact clean preregistration head. Preserve strict root `689`, request `690`, exact `689/1`, branch 7 answer `C`, checkpoint zero, N=1/T=4, no speculation, no fanout, the full pinned nine-module runtime bundle, and the 6,000 MiB WDDM ceiling.
...
- [ ] `EXECUTE_NEO_EXP_0066_STRICT_PREFIX_CUDA_RESIDENT_RAM_ROOT_N1_LATENCY`. Execute once from the exact clean preregistration head with pinned model and nine-module candidate runtime, checkpoint zero, strict `689/1`, branch 7, N=1/T=4 `CD/DC/DC/CD`, no speculation/fanout, distinct absent result/log paths, one isolated sidecar on 9494, and no retry after the first model-facing request consumes this identity.
