...
**Latest causal boundary:** `neo-exp-0063` at `lab/results.jsonl:75`, SHA-256 `C82B54699F268B0CC434F3331F95FB48C6E1D35DD7A00E4B86D9133EF3A3660F`; exact 690-token materialization, root-save, and first restore succeeded, but the first catalytic warmup forced full prompt reprocessing and returned cache `0` / fresh prompt `690` rather than preregistered `689/1`. The exact-full-prompt root mechanism is rejected; no counted pair or claim extension occurred.
...
**Identity status:** `neo-exp-0063` is scientifically consumed by its negative full-prompt-root result and cannot be retried. `neo-exp-0064` is reserved and statically preregistered for one 689-token strict-prefix N=1 latency execution; no successor live contact has occurred.
...
**Next exact action:** `EXECUTE_NEO_EXP_0064_STRICT_689_TOKEN_PREFIX_ROOT_N1_LATENCY` — from the exact clean preregistration head, invoke the dedicated strict-prefix wrapper once at checkpoint zero with pinned identities and distinct absent artifacts; require saved root 689, submitted prompt 690, catalytic 689/1, unchanged controls, and every frozen utility/latency/lifecycle gate.
...
Current evidence remains bounded: exact process-local reuse and equal-utility fanout amortization are supported through N=8; N=16 positive amortization was observed but its strict utility gate failed; direct-only controls accepted cold-chain and water at `16/16` while datacenter remains rejected at `15/16`; and one checkpoint-free N=1 temporal water-branch control supports bounded fast single-request latency through four counted pairs. Per user direction, the active research cursor continues reducing single-request catalytic latency rather than scaling fanout.
...
- [ ] `EXECUTE_NEO_EXP_0064_STRICT_689_TOKEN_PREFIX_ROOT_N1_LATENCY`. Commit the static package, verify exact clean head and candidate/model/stable/port/artifact custody, then invoke the dedicated wrapper once. Preserve every route and root operation plus the server trace; accept only exact 689 cached / 1 fresh with no forced-full-reprocessing event, identical outputs, direct-control stability, predecessor improvement, and positive complete lifecycle.
