...
**Active bounded objective:** Preserve the completed two-arm parent-dependence execution and its separate static forensic recovery. The original terminal result and archive remain permanently `INCONCLUSIVE — durable-finalization-custody-gate-failed`. Exact reconstruction reproduced failure SHA-256 `36D3843CE9B6F9A8F14C54CF139F8E505E457145C99847316C38FC2C632F32DF` and proved that the active controller-owned zero-byte `.controller.lock` was the sole postflight false negative. Both authenticated captures replayed with zero model generations: `delete-parent-0` supports Parent-A information dependence at rank zero and `3/5`; `delete-parent-1` does not show Parent-B dependence at rank zero and `5/5`. The separate artifact supports only directional Parent-A replication across the two private bindings.
**Next exact action:** Preserve this narrow directional-A recovery. Do not retry either consumed arm. Any future independent causal experiment requires a separately authorized static design and fresh authority. Parent-B dependence, bilateral parent dependence, general two-parent necessity, transfer, general catalytic inference, task advantage, superiority, SOTA, broader/restart HoloState, Deep, and automatic promotion remain locked.
...
- [x] Verify and deterministically replay both archived captures with zero model contact. Preserve Parent-A supported / Parent-B not shown, publish only `DIRECTIONAL_PARENT_A_INFORMATION_DEPENDENCE_REPLICATED_ACROSS_TWO_PRIVATE_BINDINGS`, and bind forensic artifact `lab/ck0_balanced_opaque_rank_head_v2_binding_2_parent_dependence_forensic_recovery_1.json` at SHA-256 `110DAE908E4C1F01747C2CBBEA9F3A34BB2ACA61397D55B0648D82453A3DD975`.
- [ ] Preserve this narrow directional-A recovery. Before any independent causal follow-on, obtain a separate static-design authorization and fresh authority; never retry either consumed arm.
