...
**Active bounded objective:** The minimum binding-2 parent-dependence replay is statically implemented, bounded-controller-repair resumable, and preregistered as `ck0-balanced-v2-rank-head-b2-parent-dependence-r1`. It has exactly two transform-only arms, `delete-parent-0` and `delete-parent-1`, with one future model generation per arm. Both requests reconstruct their complete retained parent from verified binding-2 archive `EBD8F01971C0D4BEE945A516D515645EF9F8F7BDC16F03D15BE7990F476536ED`; neither completed full-information run nor borrow or branch stage may run again. The frozen scientific execution surface owns the exact request bytes, model-contact path, raw capture-before-parse, source identities, and immutable one-generation-per-arm ceilings. A separately bound repairable controller may advance through at most three linear descendant-only repair commits without resetting the original authority or any per-arm consumption. Each admitted transition is observed in the authenticated append-only journal before model contact and must preserve the exact arm request hashes, frozen scientific binding, receipt, captures, preregistered scientific surface, and zero-generation replay law. Explicit captured-response invalidity may adjudicate `INCONCLUSIVE`; unexpected controller exceptions preserve captures and `finalization-observed`, append no adjudication, and raise for repair.
**Next exact action:** `After this bounded-controller-repair commit publishes, separately authorize ck0-balanced-v2-rank-head-b2-parent-dependence-r1 with one fresh external authority ID bound to the exact published commit and canonical preregistration.` Binding-2 parent dependence, causal replication across bindings, general two-parent necessity, transfer, general catalytic inference, task advantage, superiority, SOTA, broader/restart HoloState, Deep, and automatic promotion remain locked.
...
- [ ] Separately authorize only `ck0-balanced-v2-rank-head-b2-parent-dependence-r1` with one fresh external authority ID bound to the exact published bounded-controller-repair commit and canonical preregistration. Do not authorize either arm separately, any third generation, retry, full-information rerun, follow-on, or claim expansion.
