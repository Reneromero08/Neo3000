...
**Next exact action:** `STATICALLY_PREPARE_NEO_EXP_0062_CONTROLLED_RUNTIME_IDENTITY_LOCK_BINDING` — repair only the discovery-sidecar controlled-readiness identity path-lock binding while preserving experimental-binary acceptance and every water-panel scientific control. Validate statically and stop before live execution in this cycle.
...
**Failure scope law:** a safety or engineering failure stops and preserves only the affected attempt identity. It never blocks the research goal; when safely possible, select an evidence-motivated successor under a new identity and continue.
...
- [x] Execute planned `neo-exp-0061` once from exact clean frontier head `8deadf9c97f35b1ba7a1d932504803b60f18ab73` with explicit checkpoint zero under standing frontier authorization. Preserve `INCONCLUSIVE_PRE_POPEN_ENGINEERING_FAILURE`: exact binary/model verification completed, but the discovery-sidecar override did not acquire the two controlled-readiness runtime identity path locks, so `runtime-identity-path-binding-missing` stopped launch before `Popen`, PID allocation, readiness, Task-A, or any branch. No result/trace or `lab/results.jsonl` record exists; stable PID `3860`, free port `9494`, cleanup, and both clean worktrees passed. Consume this identity without retry, carrier, scaling, promotion, or same-cycle follow-on.
- [ ] `STATICALLY_PREPARE_NEO_EXP_0062_CONTROLLED_RUNTIME_IDENTITY_LOCK_BINDING`. Make one infrastructure-only correction so the discovery sidecar retains its experimental-binary identity checks while invoking the base controlled-readiness path-lock acquisition and revalidation. Add focused tests, preserve every preregistered water-panel scientific control, and do not execute the live successor in this cycle.
