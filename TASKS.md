...
**Latest successor incident:** planned `neo-exp-0060` from exact controller `b2f638654fffdfb17f25f4a3832370fc4252870e` failed closed during sidecar launch before readiness, Task-A, or any branch request when a strict stable-health probe was lost. Candidate PID `42136` retired, stable PID `3860` recovered healthy, both worktrees remained clean, no result/trace artifact exists, and no scientific record or claim change was created.
...
**Next exact action:** `EXECUTE_NEO_EXP_0061_DIRECT_ONLY_WATER_N16` — invoke the already-preregistered experiment once from the exact clean committed frontier head with the pinned identities, explicit checkpoint zero, read-only stable health contact, and one isolated candidate sidecar. Preserve no carrier, no retry for the consumed identity, no promotion, and no same-cycle follow-on.
...
- [ ] `EXECUTE_NEO_EXP_0061_DIRECT_ONLY_WATER_N16`. Verify the exact clean committed head and pinned identities, then invoke planned `neo-exp-0061` once with explicit checkpoint zero under standing frontier authorization. No carrier, scaling, retry for this identity, promotion, or same-cycle follow-on.
