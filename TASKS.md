...
**Active bounded objective:** Preserve published `neo-exp-0045`, which supports three-parent global relational-invariant extraction on two matched synthetic geometries while keeping worker synthesis, a complete catalytic lifecycle, and compute advantage unproven.
**Next exact action:** `STATIC DESIGN OF_ONE_BOUNDED_WORKER_TO_SYNTHESIS_MINI_SWARM`. Do not execute automatically, repeat synthetic transform geometry, or generalize arity. Formal algebra, full commutativity, associativity, arbitrary arity, transfer, worker synthesis, general catalytic inference, task advantage, reduced fresh computation, compute amplification, SOTA, and automatic promotion remain locked.
...
- [ ] Statically design one bounded worker-to-synthesis mini-swarm that tests whether independently generated partial worker outputs can be recombined through the evidenced relational transform into a useful synthesis result. Do not add another seed/binding replication, four-parent geometry, associativity/permutation panel, generalized n-parent infrastructure, authority, or live execution.
