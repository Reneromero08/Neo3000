...
**Active bounded objective:** Preserve accepted `neo-exp-0046` as capability evidence and preserve the current DSL carrier as capability-only. The easy Agents-A1 35B-calibrated `two-shard-semantic-xor-worker-synthesis-family-v1` now has a frozen four-task corpus, ignored protected evaluator, and statically preregistered 16-request worker-versus-baseline evaluation. Twelve worker/baseline requests are exact-hash bound; four synthesis requests remain legitimately outcome-derived under a frozen authenticated-capture derivation law. No authority, model contact, inference, capture, result, archive, or publication record exists for this evaluation; all broader claims remain locked.
**Next exact action:** `SEPARATELY_AUTHORIZE_AND_EXECUTE_TWO_SHARD_SEMANTIC_XOR_WORKER_BASELINE_EVALUATION_V1`. Bind one fresh external authority to the published static commit and exact preregistration; execute at most 16 one-generation requests with no retry or automatic follow-on.
...
- [ ] Separately authorize and execute the frozen semantic-XOR worker-versus-baseline evaluation exactly once from its published static commit. Allow at most 16 generations, no retry after request start, delayed protected scoring only after cleanup/postflight, and no automatic publication or follow-on.
