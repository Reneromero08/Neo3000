...
**Identity status:** `neo-exp-0063` is scientifically consumed by its negative full-prompt-root result and cannot be retried. `neo-exp-0064` is scientifically consumed by its accepted strict-prefix result and cannot be rerun under that identity. No `neo-exp-0065` live identity has been minted; the active cursor is zero-contact residual-latency profiling.
...
**Next exact action:** `PROFILE_DECODE_AND_RESTORE_OVERHEAD_AFTER_STRICT_PREFIX_ROOT_WITH_THE_SAME_N1_CONTROL` — use the executed 0064 artifact and pinned source to partition the residual median `0.4610`-second catalytic path into restore, one-token prompt, generation/decode, transport, and controller overhead. Make no live contact and do not optimize fanout; use the profile to preregister one causal speed intervention.
...
- [ ] `PROFILE_DECODE_AND_RESTORE_OVERHEAD_AFTER_STRICT_PREFIX_ROOT_WITH_THE_SAME_N1_CONTROL`. Partition the remaining exact catalytic path using executed 0064 request/root timings and pinned server/controller source. Preserve utility and full accounting, make no live contact during profiling, and preregister only the smallest one-variable speed intervention; do not return to fanout.
