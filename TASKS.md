...
**Active bounded objective:** Preserve published `neo-exp-0045` and both public-only corpus diagnostics. The three-worker profile is unavailable, and the separately authorized two-worker scan also found no exact profile: the six ordered pairs of distinct locally eligible shards cover at most four of five public examples, overlap on at least two examples, and retain all three support candidates in common.
**Next exact action:** `SEPARATELY_AUTHORIZE_A_STATIC_SCIENTIFIC_DECISION_ON_TWO_WORKER_PROFILE_COMPATIBILITY`. Do not create a synthetic task, weaken the frozen gates, build a live controller, or execute automatically. Worker synthesis, a complete catalytic lifecycle, task advantage, reduced fresh computation, compute amplification, general catalytic inference, SOTA, and automatic promotion remain locked.
...
- [ ] Separately decide whether to revise the two-worker profile law for this frozen corpus or stop this worker-synthesis line. Do not create a synthetic task or weaken the current structural gates without explicit authorization.
