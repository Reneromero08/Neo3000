...
**Active bounded objective:** The minimum binding-2 parent-dependence replay is statically implemented and preregistered as `ck0-balanced-v2-rank-head-b2-parent-dependence-r1`. It has exactly two transform-only arms, `delete-parent-0` and `delete-parent-1`, with one future model generation per arm. Both requests reconstruct their complete retained parent from verified binding-2 archive `EBD8F01971C0D4BEE945A516D515645EF9F8F7BDC16F03D15BE7990F476536ED`; neither completed full-information run nor borrow or branch stage may run again. One generic external authority, per-arm request-started consumption, raw capture-before-parse, keyed capture/journal custody, durable cleanup/postflight finalization, no-retry recovery, all-or-nothing two-head freezing before private mapping, and content-addressed terminal custody are bound in the preregistration.
**Next exact action:** `Separately authorize ck0-balanced-v2-rank-head-b2-parent-dependence-r1 with one fresh external authority ID bound to the exact published preparation commit and preregistration.` Binding-2 parent dependence, causal replication across bindings, general two-parent necessity, transfer, general catalytic inference, task advantage, superiority, SOTA, broader/restart HoloState, Deep, and automatic promotion remain locked.
