...
**Active bounded objective:** Execute no model contact yet. Preserve the statically preregistered four-request contrastive relational-operation probe. It uses two identity-disjoint five-by-five private geometries, one unique overlap per geometry, exact AB/BA order swaps, one fixed transform seed, deterministic rank-zero extraction, and frozen predictions for unique intersection, lexical-first, first-listed, Parent-A priority, and Parent-B priority. This is a transform-causal-core probe under one private binding, not a complete new borrow -> transform -> extract -> restore cycle.
**Next exact action:** Separately authorize exactly one live execution of `balanced-opaque-transform-only-contrastive-relational-operation-probe-v1` from the final published static commit with a fresh external authority ID. Maximum four one-generation transform requests; no retry or automatic follow-on. Formal algebra, position-independent bilateral dependence, general two-parent necessity, cross-binding generalization, transfer, general catalytic inference, task advantage, superiority, SOTA, broader/restart HoloState, Deep, and automatic promotion remain locked.
...
- [ ] Separately authorize exactly one four-request live probe from the final published static commit with a fresh external authority ID. Do not execute automatically and do not retry a started request.
