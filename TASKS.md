...
**Identity status:** `neo-exp-0061` is scientifically consumed by its accepted direct-control result. The next planned identity is `neo-exp-0062`, reserved for a statically preregistered fast single-request catalytic-latency experiment; no live successor contact has occurred.
...
**Next exact action:** `DESIGN_NEO_EXP_0062_SINGLE_REQUEST_CATALYTIC_LATENCY` — preregister the narrowest repeated temporal-reuse benchmark that compares one checkpoint-free exact reusable root against identical cache-disabled single-request controls, counts full end-to-end wall/fresh compute and closure, requires identical utility, and treats fast daily-driver inference rather than fanout as the optimization objective.
...
- [ ] `DESIGN_NEO_EXP_0062_SINGLE_REQUEST_CATALYTIC_LATENCY`. Freeze a repeated temporal-reuse, N=1 daily-driver latency discriminator on one qualified strong control. Compare exact checkpoint-free reusable-state inference against identical cache-disabled direct requests; count carrier creation separately and in full lifecycle, count restore/closure, require identical emitted-token utility and bounded residency, report median/tail end-to-end wall and fresh compute across repetitions, and introduce no new mechanism. Do not optimize for or infer from fanout.
