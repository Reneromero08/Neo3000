...
**Next exact action:** `PROFILE_NEO_EXP_0065_NGRAM_DRAFT_VERIFICATION_OVERHEAD_FROM_EXECUTED_RESULT_AND_PINNED_SOURCE` — preserve exact 689/1 reuse and the consumed negative result; localize why one accepted draft per eight candidates increased median generation from `225.1865` to `511.1520 ms` before selecting any new intervention. Make no model or endpoint contact during profiling.
...
- [ ] `PROFILE_NEO_EXP_0065_NGRAM_DRAFT_VERIFICATION_OVERHEAD_FROM_EXECUTED_RESULT_AND_PINNED_SOURCE`. Read only the result/log and pinned speculative-decoding scheduler/source. Partition the added `285.9655 ms` median generation cost, identify target verification/draft proposal/rollback/sampling boundaries that existing evidence can and cannot resolve, and select the smallest evidence-motivated speed successor without contact, retry, fanout, or claim widening.
