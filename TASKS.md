...
**Active bounded objective:** Deterministic rank-head CK0 v2 authority is versioned and its public CLI bootstrap is repaired at the static boundary. Historical run `ck0-balanced-v2-rank-head-b1-full-r1` is retired as `RETIRED_PRECONSUMPTION_COMMAND_INVOKED`: its supplied external authorization was spent by one command invocation, but no runtime authority receipt, runtime root, sidecar, model request, scientific observation, result, or closure was created. Active run order is now binding-1 `ck0-balanced-v2-rank-head-b1-full-r2`, then binding-2 `ck0-balanced-v2-rank-head-b2-full-r1`.
**Next exact action:** `Separately authorize only ck0-balanced-v2-rank-head-b1-full-r2 with a fresh external authority ID bound to the exact repaired protected commit.` The retired run and consumed historical authority ID are permanently ineligible; binding-2 remains unauthorized. End-to-end cross-binding replication, binding-2 parent dependence, causal replication, general two-parent necessity, transfer, general catalytic inference, task advantage, superiority, SOTA, broader/restart HoloState, Deep, and automatic promotion remain locked.
...
- [ ] Separately authorize only `ck0-balanced-v2-rank-head-b1-full-r2` with a fresh external authority ID bound to the exact repaired protected commit. Permit one invocation and no retry. Do not authorize or auto-follow binding-2.
