...
**Active bounded objective:** Preserve published `neo-exp-0044`, which supports the unique-intersection-like transform only under a jointly new binding-and-seed condition, and prepare the smallest three-parent non-pairwise global-invariant probe without live activity.
**Next exact action:** Statically implement and preregister only `balanced-opaque-transform-only-three-parent-global-invariant-probe-v1` with two matched geometries and six cyclic-order requests. Create no authority and perform no live execution. Formal algebra, separate seed or binding invariance, arbitrary arity, transfer, general catalytic inference, task advantage, compute amplification, SOTA, and automatic promotion remain locked.
...
- [ ] Implement and exactly preregister the bounded three-parent global-invariant probe with T0/T1 and exact cyclic orders `ABC`, `BCA`, `CAB`; reuse the source private root and seed `633514649`; create no authority or live evidence.
