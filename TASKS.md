...
**Identity status:** `neo-exp-0063` is scientifically consumed by its negative full-prompt-root result and cannot be retried. The active successor identity is `neo-exp-0064`, pending static preregistration of one 689-token strict-prefix boundary intervention; no successor live contact has occurred.
...
**Next exact action:** `PREREGISTER_NEO_EXP_0064_STRICT_689_TOKEN_PREFIX_ROOT_N1_LATENCY` — change only the materialized/saved root boundary from the exact 690-token request to `branch_tokens[:-1]` at 689 while submitting the same 690-token request; preserve all accepted 0062 controls and require 689 cached / 1 fresh without the forced-full-reprocessing trace.
...
- [ ] `PREREGISTER_NEO_EXP_0064_STRICT_689_TOKEN_PREFIX_ROOT_N1_LATENCY`. Source/trace audit identifies one-variable successor `branch_tokens[:-1]`: a 689-token restored root plus the identical 690-token submitted request makes `has_new_tokens=true`, avoids the exact-full-prompt no-cache-data reset, and should admit exactly 689 cached / 1 fresh. Preserve the materialization seed, all route controls, T=4 schedule, lifecycle costs, causal improvement gates, matched-direct drift gate, cleanup, and no-fanout scope.
