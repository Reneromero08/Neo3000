...
**Active bounded objective:** Prepare the minimum inherited Task-A carrier evaluation selected from published `neo-exp-0048`, preserving its immutable live terminal `INCONCLUSIVE` and every broader claim lock.
**Next exact action:** `PREPARE_INHERITED_TASK_A_CARRIER_EVALUATION` — freeze the four-pair evaluation that omits redundant carrier materialization and requires exact same-slot inherited-prefix reuse. Do not create authority or contact the model during preparation.
...
- [ ] Prepare the inherited Task-A carrier evaluation: preserve the same four matched pairs and delayed protected scoring; run Task A directly into cache-enabled same-slot Task B; omit four materializations; retain four closure measurements and four cache-disabled direct baselines; require exact inherited-prefix reuse before any efficiency claim. Create no authority or live state during preparation.
