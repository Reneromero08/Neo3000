...
**Active bounded objective:** Preserve accepted `neo-exp-0046` as capability evidence and preserve the current DSL carrier as capability-only. The easy Agents-A1 35B-calibrated `two-shard-semantic-xor-worker-synthesis-family-v1` is now statically admissible: two isolated natural-language judgments produce model-authored bits, one fixed XOR relation produces the final label, and a same-evidence direct baseline supplies the resource comparison. No corpus, evaluator answers, runtime, authority, or inference exists for this family; all broader claims remain locked.
**Next exact action:** `SEPARATELY_AUTHOR_CORPUS_AND_PROTECTED_EVALUATOR_FOR_TWO_SHARD_SEMANTIC_XOR_FAMILY`. Author exactly four manual task pairs covering `00 / 01 / 10 / 11` and freeze public-task and protected-evaluator hashes simultaneously. Do not implement runtime, create authority, or contact the model without later separate authorization.
...
- [ ] Separately author the four-instance public corpus and protected evaluator for `two-shard-semantic-xor-worker-synthesis-family-v1`. Cover worker-bit cells `00 / 01 / 10 / 11` exactly once, freeze both surfaces before model contact, retain every task regardless of later outcomes, and stop before runtime implementation or live preregistration.
