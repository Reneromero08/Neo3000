...
**Next exact action:** `AWAIT_SEPARATE_LIVE_STABLE_CONTACT_AUTHORIZATION_FOR_WATER_DIRECT_N16` — planned `neo-exp-0061` is statically prepared with hash-bound controlled readiness and bounded startup recovery; do not invoke it, probe the stable endpoint, or launch a sidecar until separately authorized under the active `/goal` pause gate.
...
- [ ] `AWAIT_SEPARATE_LIVE_STABLE_CONTACT_AUTHORIZATION_FOR_WATER_DIRECT_N16`. If authorized later, verify the exact clean committed head and pinned identities, then invoke planned `neo-exp-0061` once with explicit checkpoint zero. No carrier, scaling, retry, promotion, or same-cycle follow-on.
