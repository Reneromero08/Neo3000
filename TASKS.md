...
**Active bounded objective:** Preserve published `neo-exp-0044` and the prepared zero-execution three-parent global-invariant probe. The probe changes only arity and geometry while reusing the source private root, opaque correspondence, and seed.
**Next exact action:** Separately authorize exactly one live invocation of `balanced-opaque-transform-only-three-parent-global-invariant-probe-v1` from its final published static commit with one fresh external authority ID. Do not execute automatically. Formal algebra, full commutativity, associativity, arbitrary arity, transfer, general catalytic inference, task advantage, compute amplification, SOTA, and automatic promotion remain locked.
...
- [ ] Separately authorize exactly one six-request live invocation from the final published static commit with one fresh external authority ID; do not execute automatically or design a follow-on.
