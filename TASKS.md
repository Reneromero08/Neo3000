...
**Active bounded objective:** Preserve accepted `neo-exp-0046` as capability evidence and preserve the current DSL carrier as capability-only. Preserve the consumed semantic-XOR attempt from `5605c5d28a6fdbc7e1e7ee855c0515f88ad50997` as terminal `INCONCLUSIVE`: its first baseline capture exhausted the frozen eight-token ceiling before strict JSON closure, so the other 15 requests never started. The successor preregistration changes only the uniform completion ceiling from 8 to 16, covers all observed compact and pretty schema forms through 13 tokens, uses distinct attempt-2 state and authority paths, and preserves the corpus, evaluator, tasks, order, seeds, scoring, resource law, and claim locks.
**Next exact action:** `AUTHORIZE_AND_EXECUTE_TWO_SHARD_SEMANTIC_XOR_WORKER_BASELINE_EVALUATION_V1_ATTEMPT_2`. Bind one fresh external authority to the published successor commit and exact attempt-2 preregistration; execute at most 16 one-generation requests with no retry or automatic follow-on.
...
- [ ] Execute attempt 2 exactly once from its published repair commit using one fresh external authority. Allow at most 16 generations, no retry after request start, delayed protected scoring only after cleanup/postflight, and no automatic publication or follow-on.
