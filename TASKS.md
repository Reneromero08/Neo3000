...
**Latest causal boundary:** `neo-exp-0064` at `lab/results.jsonl:76`, SHA-256 `0A910B6B769B8E9F03B9A0ED8E67EAE8FF1BF6C8696E94B583FFFB69E9A15C94`; the 689-token strict-prefix root avoided forced full reprocessing, admitted exact cache/fresh `689/1`, preserved exact output through four counted pairs, and reduced median restore-inclusive effective wall from `10.7735` direct to `0.4610` seconds (`23.370x`). The charged T=4 lifecycle improved `2.122x` wall and `2.554x` fresh compute.
...
**Identity status:** `neo-exp-0063` is scientifically consumed by its negative full-prompt-root result and cannot be retried. `neo-exp-0064` is scientifically consumed by its accepted strict-prefix result and cannot be rerun under that identity. `neo-exp-0065` is statically preregistered and unconsumed; no successor live contact has occurred.
...
**Next exact action:** `EXECUTE_NEO_EXP_0065_STRICT_PREFIX_PLUS_PROCESS_LOCAL_NGRAM_CACHE_N1_LATENCY` — from the exact clean preregistration head, launch the pinned isolated candidate once at checkpoint zero and change only speculative type `none` to `ngram-cache`. Preserve the exact 689/1 root, output identity, T=4 controls, two charged n-gram-mutating warmups, bounded memory, and process-retirement closure; preserve and adjudicate every outcome without retrying a consumed identity.
...
- [ ] `EXECUTE_NEO_EXP_0065_STRICT_PREFIX_PLUS_PROCESS_LOCAL_NGRAM_CACHE_N1_LATENCY`. Execute once from the exact clean preregistration head with the pinned candidate/model, checkpoint zero, one isolated sidecar on 9494, distinct absent result/log paths, and stable PID 3860 health-only on 9292. Preserve every accepted and rejected outcome; no same-ID retry, carrier scaling, fanout, promotion, or stable mutation.
