...
**Active bounded objective:** Preserve the published inherited Task-A carrier result and retired same-panel line while holding the four-root, eight-branch runtime-native carrier evaluation frozen and unexecuted under every existing claim lock.
...
- [ ] Separately authorize exactly one live `holostate-v1-multi-branch-runtime-native-carrier-evaluation-v1` execution from its published preparation commit. Create no authority or runtime state before that authorization.
